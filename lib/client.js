...
`;
		async function call(action) {
			const timeout = action.kind === "add-project-repository" ? 75e3 : 2e4;
      ...
      <section class="dsh-sdd-card"><h2>阶段进度</h2><div class="dsh-sdd-stage-grid">${dashboard.stages.map((item) => {
					return `<div class="dsh-sdd-stat"><div class="dsh-sdd-stage-head"><strong>${STAGES.find((value) => value.id === item.stage).label}</strong><span class="dsh-sdd-badge">${item.status}</span></div><div class="dsh-sdd-progress"><span style="width:${item.completion}%"></span></div><span class="dsh-sdd-muted">${item.completion}% · draft ${item.drafts} · accepted ${item.accepted}</span></div>`;
				}).join("")}</div></section>
      <div class="dsh-sdd-grid dsh-sdd-dashboard-columns" style="margin-top:14px"><section class="dsh-sdd-card"><h2>质量与追踪</h2><p>来源追踪覆盖率：<strong>${dashboard.traceability}%</strong></p>${dashboard.blockers.length === 0 ? "<div class=\"dsh-sdd-empty\">当前没有结构化阻塞项</div>" : `<ul class="dsh-sdd-checks">${dashboard.blockers.map((item) => `<li data-fail>${escapeHtml(item)}</li>`).join("")}</ul>`}</section><section class="dsh-sdd-card"><h2>最近活动</h2>${dashboard.recentEvents.length === 0 ? "<div class=\"dsh-sdd-empty\">暂无事件</div>" : `<div class="dsh-sdd-list dsh-sdd-scroll-list">${dashboard.recentEvents.slice(0, 10).map((event) => `<div class="dsh-sdd-row"><span></span><span><strong>${escapeHtml(event.subject)}</strong><span class="dsh-sdd-muted">${escapeHtml(event.type)} · ${escapeHtml(event.time)}</span></span></div>`).join("")}</div>`}</section></div>${this.traceabilityHtml(snapshot)}`;
			...
			outputRow(item, snapshot) {
				...
				return `<div class="dsh-sdd-row">${selectable ? `<input type="radio" name="sdd-target" data-target="${escapeHtml(item.uid)}" ${this.state.targetArtifactUid === item.uid ? "checked" : ""}>` : "<span></span>"}<span><strong>${escapeHtml(item.key)} · ${escapeHtml(item.title)}</strong><span class="dsh-sdd-muted">v${escapeHtml(item.version)} · ${escapeHtml(item.relativeDirectory)}/${escapeHtml(item.entry)} · 文件 ${item.files.length}</span>${checks}<div class="dsh-sdd-actions"><button class="dsh-sdd-button" data-preview-artifact="${escapeHtml(item.uid)}">查看交付包</button><button class="dsh-sdd-button" data-quality="${escapeHtml(item.uid)}">质量检查 ${report?.score ?? 0}%</button>${run?.sessionId ? `<button class="dsh-sdd-button" data-resume="${escapeHtml(run.uid)}">恢复对话</button><button class="dsh-sdd-button" data-sync="${escapeHtml(run.uid)}">同步结论</button>` : ""}${selectable ? `<button class="dsh-sdd-button" data-accept="${escapeHtml(item.uid)}">验收</button><button class="dsh-sdd-button" data-discard="${escapeHtml(item.uid)}">删除草稿</button>` : ""}${item.status === "accepted" ? `<button class="dsh-sdd-button" data-revision="${escapeHtml(item.uid)}">发起变更（新修订）</button>` : ""}${run !== void 0 && item.status === "accepted" ? `<button class="dsh-sdd-button" data-complete="${escapeHtml(run.uid)}">完成阶段运行</button>` : ""}</div></span><span><span class="dsh-sdd-badge">${escapeHtml(item.status)}</span>${run ? `<span class="dsh-sdd-badge">${escapeHtml(run.status)}</span>` : ""}</span></div>`;
			}
			...
			async addProjectRepository() {
				const values = await this.openForm({
					title: "添加项目代码仓库",
					description: "添加时只验证来源和基线分支，不复制或下载代码。添加成功后会立即显示；开发阶段才为具体需求创建本地 Worktree 或远程 Clone。测试命令仍可在 project.yaml 中补充。",
					submitLabel: "验证并添加",
					fields: [
						{
							name: "id",
