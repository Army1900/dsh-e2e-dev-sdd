...
`;
		async function call(action) {
			const timeout = action.kind === "development-install-openspec" ? 33e4 : action.kind === "development-initialize-openspec" ? 21e4 : action.kind === "development-fork-openspec-schema" || action.kind === "development-create-openspec-change" || action.kind === "development-inspect-openspec-templates" ? 75e3 : action.kind === "project-git-fetch" || action.kind === "project-git-sync" || action.kind === "project-git-push" ? 21e4 : action.kind === "add-project-repository" || action.kind === "inspect-project-repository" || action.kind === "initialize-project-repository" || action.kind === "update-project-repository-branch" ? 75e3 : 2e4;
		...
		var SddWorkbench = class {
			...
			reconcileSelection(snapshot) {
				if (this.state.workItemUid === void 0 || !snapshot.workItems.some((item) => item.uid === this.state.workItemUid)) {
			...
			defaultInputs(snapshot, stage) {
				...
				const selected = new Set([workItem?.sourceUid, workItem?.bundleSourceUid].filter((uid) => uid !== void 0));
			...
			render() {
				...
				const workItemSelect = snapshot !== void 0 && snapshot.workItems.length > 0 && this.state.menu !== "settings" ? `<select class="dsh-sdd-select" data-action="work-item" title="当前需求工作单元">${workItemOptions}</select>` : "";
				let body = "";
