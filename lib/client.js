...
`;
      ...
      <div class="dsh-sdd-grid dsh-sdd-dashboard-columns" style="margin-top:14px"><section class="dsh-sdd-card"><h2>质量与追踪</h2><p>来源追踪覆盖率：<strong>${dashboard.traceability}%</strong></p>${dashboard.blockers.length === 0 ? "<div class=\"dsh-sdd-empty\">当前没有结构化阻塞项</div>" : `<ul class="dsh-sdd-checks">${dashboard.blockers.map((item) => `<li data-fail>${escapeHtml(item)}</li>`).join("")}</ul>`}</section><section class="dsh-sdd-card"><h2>最近活动</h2>${dashboard.recentEvents.length === 0 ? "<div class=\"dsh-sdd-empty\">暂无事件</div>" : `<div class="dsh-sdd-list dsh-sdd-scroll-list">${dashboard.recentEvents.slice(0, 10).map((event) => `<div class="dsh-sdd-row"><span></span><span><strong>${escapeHtml(event.subject)}</strong><span class="dsh-sdd-muted">${escapeHtml(event.type)} · ${escapeHtml(event.time)}</span></span></div>`).join("")}</div>`}</section></div>${this.traceabilityHtml(snapshot)}`;
			...
			stageSettingsHtml(snapshot, stage) {
				...
				const openSpecState = openSpecValidation?.status === "valid" ? openSpecValidation.message : openSpecValidation?.status === "invalid" ? `验证失败：${openSpecValidation.message}` : openSpecValidation?.status === "pending" ? "已配置，待开发空间验证" : "已配置，待验证";
				...
				const openSpecReadyActions = stage === "development" && openSpecValidation?.status === "valid" ? [
					...
					openSpecValidation.schema !== void 0 && openSpecValidation.schema !== "spec-driven" ? "<button class=\"dsh-sdd-button\" data-action=\"open-openspec-schema\">打开 Schema 目录</button>" : "",
				...
				const description = stage === "architecture" ? "系统设计角色确认本需求涉及的代码仓库范围。项目仓库目录和默认基线请在“项目设置”维护。" : stage === "specification" ? "规格设计角色在已确认仓库范围内明确实际修改仓库和具体开发目标。" : "开发阶段只执行最终开发配置；跳过前置阶段时，可以在这里补齐缺失的仓库范围和开发目标。";
			...
			async configureDevelopmentTargets() {
				...
				const values = await this.openForm({
					...
					description: configureOpenSpec ? "选择本需求实际修改的仓库，并填写具体改动目标；开发阶段还可以选择是否使用 OpenSpec。" : "选择本需求实际修改的仓库，并为每个选中的仓库填写具体、可交付的改动目标。",
					submitLabel: "保存开发设置",
					fields: [...scope.flatMap((repository) => [{
						...
						},
						{
							...
							type: "text",
							value: workItem.openSpec?.schema ?? "spec-driven",
							placeholder: "例如：spec-driven 或 company-sdd"
						}
					] : []]
				});
				if (values === void 0) return;
				...
				const openSpec = configureOpenSpec ? values.openSpecEnabled === true ? {
					enabled: true,
					repositoryId: String(values.openSpecRepository),
					path: String(values.openSpecPath),
					schema: String(values.openSpecSchema),
					...workItem.openSpec?.changeId === void 0 ? {} : { changeId: workItem.openSpec.changeId }
				} : { enabled: false } : workItem.openSpec ?? { enabled: false };
				await this.mutate({
					kind: "update-work-item-settings",
					workspaceId: this.state.workspaceId,
					workItemUid: workItem.uid,
			...
			async disableOpenSpec() {
				...
				await this.mutate({
					...
					workItemUid: workItem.uid,
					repositoryScope: workItem.repositoryScope ?? [],
					developmentTargets: workItem.developmentTargets ?? [],
					developmentTargetDetails: workItem.developmentTargetDetails,
