...
Please report this to https://github.com/markedjs/marked.`, e) {
		...
		//#endregion
		//#region src/client/json-preview.ts
		...
		function node(value, path, depth, options) {
			...
			if (typeof value === "string") {
				...
				return `<div class="dsh-sdd-json-html">${options.sanitizeHtml(value)}</div><details class="dsh-sdd-json-html-source"><summary>查看 HTML 源码</summary><pre class="dsh-sdd-template-preview">${options.escapeHtml(value)}</pre></details>`;
			}
			if (value === void 0) return "<span class=\"dsh-sdd-json-literal\">undefined</span>";
		...
		function jsonPreviewHtml(value, options) {
			return node(value, [], 0, options);
		}
		//#endregion
		//#region src/client/source-selection.ts
		/** Prefer explicit enterprise providers and available Connectors over zero-configuration manual intake. */
		function preferredSourceSelection(providers, connectors, configured) {
			const configuredProvider = configured !== void 0 && providers.includes(configured.provider) ? configured.provider : void 0;
			const customProvider = providers.find((provider) => provider !== "manual" && provider !== "command");
			const commandProvider = providers.includes("command") && connectors.length > 0 ? "command" : void 0;
			const provider = configuredProvider !== void 0 && configuredProvider !== "manual" ? configuredProvider : customProvider ?? commandProvider ?? configuredProvider ?? (providers.includes("manual") ? "manual" : providers[0]);
			const connector = (configured?.connector !== void 0 && connectors.some((connector) => connector.id === configured.connector) ? configured.connector : void 0) ?? connectors.find((item) => item.scope === "project")?.id ?? connectors[0]?.id;
			return {
				provider,
...
`;
		async function call(action) {
			const timeout = action.kind === "bind-session" || action.kind === "context" ? 33e4 : action.kind === "development-install-openspec" ? 33e4 : action.kind === "development-initialize-openspec" ? 21e4 : action.kind === "development-fork-openspec-schema" || action.kind === "development-create-openspec-change" || action.kind === "development-inspect-openspec-templates" ? 75e3 : action.kind === "project-git-fetch" || action.kind === "project-git-sync" || action.kind === "project-git-push" ? 21e4 : action.kind === "add-project-repository" || action.kind === "inspect-project-repository" || action.kind === "initialize-project-repository" || action.kind === "update-project-repository-branch" ? 75e3 : 2e4;
		...
		function stageMenu(menu) {
			return STAGES.some((stage) => stage.id === menu);
		}
		...
		function sanitizedRichHtml(value) {
		...
		function mountJsonPreview(container, value) {
			const expanded = /* @__PURE__ */ new Set();
			const render = () => {
				container.innerHTML = jsonPreviewHtml(value, {
					escapeHtml,
					sanitizeHtml: sanitizedRichHtml,
					expanded
				});
				container.querySelectorAll("[data-json-expand]").forEach((button) => button.addEventListener("click", () => {
					expanded.add(button.dataset.jsonExpand);
					render();
				}));
			};
			render();
		...
		var SddWorkbench = class {
			...
			open(menu) {
				...
				if (snapshot !== void 0 && snapshot.workspace.workspaceId === this.state.workspaceId) {
			...
			reconcileSelection(snapshot) {
				...
				if (this.state.workItemUid === void 0 || !deliveryWorkItems.some((item) => item.uid === this.state.workItemUid)) {
					const workItem = deliveryWorkItems.find((item) => item.status !== "completed");
					this.state.workItemUid = workItem?.uid;
					this.state.selected = workItem === void 0 ? /* @__PURE__ */ new Set() : this.workItemSourceUids(snapshot, workItem.uid, stageMenu(this.state.menu) ? this.state.menu : void 0);
					this.state.targetArtifactUid = void 0;
				}
			...
			defaultInputs(snapshot, stage) {
				const workItem = snapshot.workItems.find((item) => item.uid === this.state.workItemUid);
				const selected = workItem === void 0 ? /* @__PURE__ */ new Set() : this.workItemSourceUids(snapshot, workItem.uid, stage);
				...
				for (const upstream of STAGES.slice(0, stageIndex)) {
					...
					if (latest !== void 0) selected.add(latest.uid);
				}
				return selected;
			}
			workItemSourceUids(snapshot, workItemUid, stage) {
			...
			render() {
				...
				const workItemSelect = snapshot !== void 0 && deliveryWorkItems.length > 0 && this.state.menu !== "settings" ? `<select class="dsh-sdd-select" data-action="work-item" title="当前交付工作单元">${workItemOptions}</select>` : "";
				let body = "";
				if (this.state.loading && snapshot === void 0) body = "<div class=\"dsh-sdd-empty\">正在读取 SDD 项目…</div>";
				...
				else if (snapshot !== void 0) body = this.state.menu === "dashboard" ? this.dashboardHtml(snapshot) : this.state.menu === "settings" ? this.settingsHtml(snapshot) : this.workbenchHtml(snapshot, this.state.menu);
				const busy = this.state.loading && snapshot !== void 0 ? "<div class=\"dsh-sdd-busy\" role=\"status\">正在应用变更并更新项目状态，当前页面可以继续查看…</div>" : "";
			...
			updateBusy(active, message = "正在应用变更并更新项目状态，当前页面可以继续查看…") {
				this.state.loading = active;
				const page = this.container?.querySelector(".dsh-sdd-page");
				...
				busy.className = "dsh-sdd-busy";
				busy.dataset.sddBusy = "";
				busy.role = "status";
      ...
      <div class="dsh-sdd-grid dsh-sdd-dashboard-columns" style="margin-top:14px"><section class="dsh-sdd-card"><h2>质量与追踪</h2><p>来源追踪覆盖率：<strong>${dashboard.traceability}%</strong></p>${dashboard.blockers.length === 0 ? "<div class=\"dsh-sdd-empty\">当前没有结构化阻塞项</div>" : `<ul class="dsh-sdd-checks">${dashboard.blockers.map((item) => `<li data-fail>${escapeHtml(item)}</li>`).join("")}</ul>`}</section><section class="dsh-sdd-card"><h2>最近活动</h2>${dashboard.recentEvents.length === 0 ? "<div class=\"dsh-sdd-empty\">暂无事件</div>" : `<div class="dsh-sdd-list dsh-sdd-scroll-list">${dashboard.recentEvents.slice(0, 10).map((event) => `<div class="dsh-sdd-row"><span></span><span><strong>${escapeHtml(event.subject)}</strong><span class="dsh-sdd-muted">${escapeHtml(event.type)} · ${escapeHtml(event.time)}</span></span></div>`).join("")}</div>`}</section></div>${this.traceabilityHtml(snapshot)}`;
			...
			productKnowledgeHtml(snapshot) {
				const product = snapshot.productKnowledge;
				const features = product?.features ?? [];
				const deliveries = product?.deliveries ?? [];
				const featureRows = features.slice(0, 5).map((feature) => `<div class="dsh-sdd-row"><span></span><span><strong>${escapeHtml(feature.key)} · ${escapeHtml(feature.name)}</strong><span class="dsh-sdd-muted">v${escapeHtml(feature.currentVersion)} · ${feature.status === "active" ? "当前有效" : "已废弃"} · ${feature.history.length} 次生命周期记录</span></span><span><button class="dsh-sdd-button" data-product-file="${escapeHtml(`${feature.relativeDirectory}/feature.md`)}" data-product-title="${escapeHtml(`${feature.key} · ${feature.name}`)}">查看特性</button></span></div>`).join("");
				const deliveryRows = deliveries.slice(0, 5).map((delivery) => `<div class="dsh-sdd-row"><span></span><span><strong>${escapeHtml(delivery.key)} · ${escapeHtml(delivery.workItemKey)} ${escapeHtml(delivery.title)}</strong><span class="dsh-sdd-muted">${escapeHtml(delivery.archivedAt)} · 特性 ${escapeHtml(delivery.featureRefs.map((item) => `${item.key} v${item.version}`).join("、"))}</span></span><span><button class="dsh-sdd-button" data-product-file="${escapeHtml(`${delivery.relativeDirectory}/${delivery.reportPath}`)}" data-product-title="转测报告 · ${escapeHtml(delivery.key)}">报告</button> <button class="dsh-sdd-button" data-product-file="${escapeHtml(`${delivery.relativeDirectory}/${delivery.emailPath}`)}" data-product-title="转测邮件 · ${escapeHtml(delivery.key)}">邮件稿</button></span></div>`).join("");
				return `<section class="dsh-sdd-card" style="margin-bottom:14px"><h2>产品积累</h2><p class="dsh-sdd-muted">需求完成后沉淀为项目根目录中的长期产品资产；过程数据仍保留在 .sdd 中。</p><div class="dsh-sdd-actions"><button class="dsh-sdd-button" data-product-file="${escapeHtml(product?.catalogPath ?? "product/feature-catalog.md")}" data-product-title="产品特性目录"${features.length === 0 ? " disabled" : ""}>产品特性目录</button><button class="dsh-sdd-button primary" data-product-file="${escapeHtml(product?.specificationPath ?? "product/product-specification.md")}" data-product-title="产品当前规格"${features.length === 0 ? " disabled" : ""}>产品当前规格</button></div><div class="dsh-sdd-grid" style="margin-top:12px"><div><strong>当前特性（${features.length}）</strong><div class="dsh-sdd-list" style="margin-top:8px">${featureRows || "<div class=\"dsh-sdd-empty\">完成首个需求交付后开始积累</div>"}</div></div><div><strong>交付归档（${deliveries.length}）</strong><div class="dsh-sdd-list" style="margin-top:8px">${deliveryRows || "<div class=\"dsh-sdd-empty\">尚无已归档交付</div>"}</div></div></div></section>`;
			}
			defectOverviewHtml(snapshot) {
				const defects = snapshot.dashboard.defects;
				const rows = [
					[
						"交付已覆盖",
						defects.deliveryCovered,
						defects.total
					],
					[
						"交付待覆盖",
						defects.deliveryPending,
						defects.total
					],
					[
						"业务状态已解决",
				...
				return `<section class="dsh-sdd-card"><h2>缺陷交付概览</h2><p class="dsh-sdd-muted">独立缺陷走自己的流程；需求内缺陷由所属需求的开发交付覆盖。</p><div class="dsh-sdd-overview-list">${defects.total === 0 ? "<div class=\"dsh-sdd-empty\">暂无缺陷</div>" : rows.map(([label, value, total]) => `<div class="dsh-sdd-overview-row"><span>${escapeHtml(label)}</span><div class="dsh-sdd-progress"><span style="width:${total === 0 ? 0 : Math.round(value / total * 100)}%"></span></div><strong>${value} / ${total}</strong></div>`).join("")}</div></section>`;
			}
			workloadOverviewHtml(snapshot) {
				const workload = snapshot.dashboard.workload;
				return `<section class="dsh-sdd-card"><h2>外部工作量</h2><p class="dsh-sdd-muted">按业务来源中的估算单位汇总，仅反映外部系统状态。</p><div class="dsh-sdd-overview-list">${workload.length === 0 ? "<div class=\"dsh-sdd-empty\">业务适配器尚未提供工作量估算</div>" : workload.map((item) => `<div class="dsh-sdd-overview-row" title="${escapeHtml(item.unit)} · 已完成 ${item.completed} / 总计 ${item.total}"><span>${escapeHtml(item.unit)}</span><div class="dsh-sdd-progress"><span style="width:${item.total === 0 ? 0 : Math.min(100, Math.round(item.completed / item.total * 100))}%"></span></div><strong>${item.completed} / ${item.total}</strong></div>`).join("")}</div></section>`;
			}
			settingsHtml(snapshot) {
				const project = snapshot.project;
				if (project === void 0) return "";
				const rows = project.development.repositories.map((repository) => `<div class="dsh-sdd-row"><span></span><span><strong>${escapeHtml(repository.id)}</strong><span class="dsh-sdd-muted">${escapeHtml(repository.source)}<br>默认基线：${escapeHtml(repository.baseBranch)}</span></span><span><button class="dsh-sdd-button" data-change-repository-branch="${escapeHtml(repository.id)}">切换基线</button> <button class="dsh-sdd-button" data-remove-repository="${escapeHtml(repository.id)}">移除</button></span></div>`).join("");
				...
				const gitState = git?.isRepository !== true ? "未初始化 Git 仓库" : !git.exactWorkspaceRoot ? `工作空间位于仓库 ${git.repositoryRoot ?? ""} 内，但不是仓库根目录` : `${git.branch ?? "detached HEAD"} @ ${git.headCommit?.slice(0, 8) ?? "无提交"} · ${git.changedFiles} 个变更（暂存 ${git.stagedFiles}、未跟踪 ${git.untrackedFiles}）· ahead ${git.ahead} / behind ${git.behind}`;
			...
			stageFlowHtml(snapshot) {
				...
				const rows = snapshot.dashboard.stageFlow.map((flow) => {
					...
					const values = {
						...
						"not-applicable": flow.notApplicable,
			...
			deliveryMatrixHtml(snapshot) {
				...
				const typeLabel = (kind) => kind === "requirement" ? "需求" : kind === "defect" ? "独立缺陷" : kind;
			...
			workbenchHtml(snapshot, stage) {
				...
				const sourceUids = workItem === void 0 ? /* @__PURE__ */ new Set() : this.workItemSourceUids(snapshot, workItem.uid, stage);
				...
				const attachedDefects = workItem === void 0 ? [] : snapshot.workItems.filter((item) => item.executionMode === "attached" && item.parentWorkItemUid === workItem.uid);
				...
				let applicabilityHtml = workItem === void 0 || stage === "requirements" ? "" : applicability?.status === "not-applicable" ? `<div class="dsh-sdd-card" style="margin-bottom:14px"><strong>本需求已将${escapeHtml(STAGES.find((item) => item.id === stage).label)}标记为不适用</strong><p class="dsh-sdd-muted">${escapeHtml(applicability.reason || "未填写说明")}</p><button class="dsh-sdd-button" data-action="restore-stage">恢复为可用阶段</button></div>` : current.length === 0 ? `<div class="dsh-sdd-actions" style="margin-bottom:14px"><button class="dsh-sdd-button" data-action="skip-stage">本需求不需要${escapeHtml(STAGES.find((item) => item.id === stage).label)}</button></div>` : "";
				...
				const codeRun = target === void 0 ? void 0 : snapshot.runs.find((item) => item.artifactUid === target.uid && item.status !== "completed");
				const codeReferenceHtml = stage === "development" || configuredRepositories.length === 0 ? "" : codeRun?.codeReferences === void 0 ? `<div class="dsh-sdd-muted" style="margin-top:12px"><strong>代码参考：</strong>项目已关联 ${configuredRepositories.length} 个仓库；开始对话时自动准备为只读辅助输入，无需逐阶段选择。</div>` : `<div class="dsh-sdd-muted" style="margin-top:12px"><strong>代码参考：</strong>${codeRun.codeReferences.map((reference) => reference.available && reference.baseCommit !== void 0 ? `${reference.repositoryId} @ ${reference.baseCommit.slice(0, 8)}` : `${reference.repositoryId}（暂不可用）`).join("、") || "无"}</div>`;
				...
				return `${change}${noWorkItem}${attachedDefectsHtml}${applicabilityHtml}${this.stageSettingsHtml(snapshot, stage)}<div class="dsh-sdd-grid"><section class="dsh-sdd-card"><h2>本阶段输入材料（${selectedArtifacts.length + selectedSources.length}）</h2><p class="dsh-sdd-muted">${target === void 0 ? "创建草稿前选择输入；默认推荐当前来源、需求内缺陷和每个上游阶段的最新已验收版本。" : `输入已固定在 ${target.key} v${target.version} 的 manifest.yaml 中，调整上游输入需要创建修订。`}</p><div class="dsh-sdd-list dsh-sdd-bounded-list dsh-sdd-input-summary">${selectedRows || "<div class=\"dsh-sdd-empty\">尚未选择输入材料</div>"}</div>${codeReferenceHtml}<div class="dsh-sdd-actions"><button class="dsh-sdd-button" data-action="select-inputs"${target === void 0 ? "" : " disabled"}>${target === void 0 ? "选择/调整输入" : "输入已固定"}</button>${importAction}</div></section><section class="dsh-sdd-card"><h2>${escapeHtml(deliverableName)}</h2><p class="dsh-sdd-muted">当前处理中和最近已验收版本优先展示，其余版本收纳到历史记录。</p>${outputList}<div class="dsh-sdd-muted" style="margin-top:12px">${escapeHtml(nextStep)}</div><div class="dsh-sdd-actions"><button class="dsh-sdd-button" data-action="view-template">查看${escapeHtml(deliverableName)}模板</button><button class="dsh-sdd-button${target === void 0 ? " primary" : ""}" data-action="draft"${snapshot.workItems.some((item) => item.executionMode !== "attached") && (workItem === void 0 || applicability?.status === "not-applicable") ? " disabled" : ""}>创建${escapeHtml(deliverableName)}草稿</button><button class="dsh-sdd-button primary" data-action="conversation"${target === void 0 ? " disabled title=\"请先创建或选择本阶段交付件草稿\"" : ""}>开始阶段对话</button></div></section>${stage === "development" ? `${this.developmentHtml(snapshot)}${this.deliveryCloseoutHtml(snapshot)}` : ""}</div>`;
			}
			deliveryCloseoutHtml(snapshot) {
				const workItem = snapshot.workItems.find((item) => item.uid === this.state.workItemUid);
				if (workItem === void 0 || workItem.executionMode === "attached") return "";
				const archive = snapshot.productKnowledge?.deliveries.find((item) => item.workItemUid === workItem.uid);
				if (archive !== void 0) return `<section class="dsh-sdd-card dsh-sdd-wide"><h2>需求交付已归档</h2><p class="dsh-sdd-muted">${escapeHtml(archive.key)} 已沉淀到 ${escapeHtml(archive.featureRefs.map((item) => `${item.key} v${item.version}`).join("、"))}，历史阶段成果和转测材料已冻结。</p><div class="dsh-sdd-actions"><button class="dsh-sdd-button primary" data-product-file="${escapeHtml(`${archive.relativeDirectory}/${archive.reportPath}`)}" data-product-title="转测报告 · ${escapeHtml(archive.key)}">查看转测报告</button><button class="dsh-sdd-button" data-product-file="${escapeHtml(`${archive.relativeDirectory}/${archive.emailPath}`)}" data-product-title="转测邮件 · ${escapeHtml(archive.key)}">查看邮件稿</button></div></section>`;
				const accepted = snapshot.artifacts.some((item) => item.workItemUid === workItem.uid && item.stage === "development" && item.status === "accepted");
				const drafts = snapshot.artifacts.some((item) => item.workItemUid === workItem.uid && (item.status === "draft" || item.status === "in-review"));
				const ready = accepted && !drafts && workItem.status === "active";
				return `<section class="dsh-sdd-card dsh-sdd-wide"><h2>需求交付收口</h2><p class="dsh-sdd-muted">${escapeHtml(!accepted ? "开发测试交付记录验收后才能归档。" : drafts ? "请先验收或删除仍在处理的草稿。" : workItem.status !== "active" ? "请先处理外部来源变化。" : "归档将冻结历史交付件，更新产品特性和当前规格，并生成转测报告及邮件稿。")}</p><button class="dsh-sdd-button primary" data-action="close-delivery"${ready ? "" : " disabled"}>归档并沉淀到产品</button></section>`;
			}
			openSpecCardHtml(snapshot, stage) {
				const workItem = snapshot.workItems.find((item) => item.uid === this.state.workItemUid);
				if (workItem === void 0) return "";
				const focus = {
					requirements: "把目标、范围和验收场景沉淀到 proposal / specs",
					prototype: "把用户流程、交互状态和原型约束沉淀到 design",
					architecture: "把系统边界、接口、数据和关键决策沉淀到 design",
					specification: "把可实现、可验证的规则沉淀到 specs / tasks",
					development: "按照 tasks 实施，持续同步代码与验证结果"
				};
				if (workItem.openSpec?.enabled !== true) return `<section class="dsh-sdd-card" style="margin-bottom:14px"><h2>OpenSpec · 跨阶段规格</h2><p class="dsh-sdd-muted">启用后，AI 会在每轮对话中同时维护阶段成果和 OpenSpec，您不需要记命令或重复转述结论。本阶段重点：${escapeHtml(focus[stage])}。</p><div class="dsh-sdd-actions"><button class="dsh-sdd-button primary" data-action="enable-openspec">为本需求启用 OpenSpec</button></div></section>`;
				const validation = snapshot.openSpecValidation[workItem.uid];
				const ready = validation?.status === "valid";
				const artifacts = validation?.artifacts?.map((item) => `<span class="dsh-sdd-badge">${escapeHtml(item.id)} · ${escapeHtml(item.status)}</span>`).join("") ?? "";
				const editableFiles = (validation?.files ?? []).filter((path) => /^(?:config\.ya?ml|schemas\/|changes\/)/.test(path));
				const files = editableFiles.length === 0 ? "" : `<details class="dsh-sdd-history"><summary>管理配置、Schema 与 Change 文件（${editableFiles.length}）</summary><div class="dsh-sdd-list dsh-sdd-bounded-list" style="margin-top:8px">${editableFiles.map((path) => `<div class="dsh-sdd-row"><span></span><span><strong>${escapeHtml(path)}</strong></span><span><button class="dsh-sdd-button" data-openspec-edit="${escapeHtml(path)}">查看 / 编辑</button></span></div>`).join("")}</div></details>`;
				const nextAction = validation?.canInstall === true ? "<button class=\"dsh-sdd-button primary\" data-action=\"install-openspec\">安装 OpenSpec CLI</button>" : validation?.canInitialize === true || validation === void 0 ? "<button class=\"dsh-sdd-button primary\" data-action=\"initialize-managed-openspec\">初始化规划工作区</button>" : ready && validation.changeExists !== true ? "<button class=\"dsh-sdd-button primary\" data-action=\"create-managed-openspec-change\">创建当前需求 Change</button>" : "";
				return `<section class="dsh-sdd-card" style="margin-bottom:14px"><h2>OpenSpec · ${escapeHtml(workItem.key)}</h2><p class="dsh-sdd-muted"><strong>${escapeHtml(validation?.message ?? "正在检查")}</strong><br>当前位置：${validation?.workspace === "development" ? "隔离开发工作区" : "项目规划工作区"}${validation?.relativeRoot ? ` · ${escapeHtml(validation.relativeRoot)}` : ""}<br>本阶段重点：${escapeHtml(focus[stage])}。AI 会同步更新，无需您重复输入 OpenSpec 命令。</p>${artifacts ? `<div>${artifacts}</div>` : ""}${files}<div class="dsh-sdd-actions">${nextAction}${ready ? "<button class=\"dsh-sdd-button\" data-action=\"validate-managed-openspec\">校验全部规格</button><button class=\"dsh-sdd-button\" data-action=\"fork-managed-openspec-schema\">定制 Schema</button><button class=\"dsh-sdd-button\" data-action=\"open-managed-openspec\">打开工作区</button>" : ""}<button class="dsh-sdd-button" data-action="disable-openspec">关闭关联</button></div></section>`;
			}
			stageSettingsHtml(snapshot, stage) {
				...
				const openSpecActions = "";
				const openSpecReadyActions = "";
				const description = stage === "architecture" ? "系统设计角色确认本需求涉及的代码仓库范围。项目仓库目录和默认基线请在“项目设置”维护。" : stage === "specification" ? "规格设计角色在已确认仓库范围内明确实际修改仓库和具体开发目标。" : "开发阶段只执行最终开发配置；跳过前置阶段时，可以在这里补齐缺失的仓库范围和开发目标。规划方法由插件在后台自动准备。";
				const buttons = stage === "architecture" ? `<button class="dsh-sdd-button primary" data-action="configure-scope"${repositories.length === 0 ? " disabled" : ""}>确认仓库范围</button>` : `<button class="dsh-sdd-button" data-action="configure-scope"${repositories.length === 0 ? " disabled" : ""}>${workItem.repositoryScope?.length ? "调整仓库范围" : "补充仓库范围"}</button><button class="dsh-sdd-button primary" data-action="configure-targets"${workItem.repositoryScope?.length ? "" : " disabled"}>配置开发目标</button>${stage === "development" ? `${openSpecActions}${openSpecReadyActions}` : ""}`;
				return `<section class="dsh-sdd-card" style="margin-bottom:14px"><h2>${stage === "architecture" ? "仓库范围" : stage === "specification" ? "开发目标" : "开发执行设置"}</h2><p class="dsh-sdd-muted">${description}</p><p class="dsh-sdd-muted">仓库范围：${escapeHtml(scope)}　开发目标：${escapeHtml(targets)}</p><div class="dsh-sdd-actions">${buttons}</div></section>`;
			}
			inputSummaryRow(item) {
				return `<div class="dsh-sdd-row"><span></span><span><strong>${escapeHtml(item.key)} · ${escapeHtml(item.title)}</strong><span class="dsh-sdd-muted">${escapeHtml(STAGES.find((stage) => stage.id === item.stage)?.label ?? item.stage)} · 固定版本 v${escapeHtml(item.version)}</span></span><span><span class="dsh-sdd-badge">已验收</span><button class="dsh-sdd-button" data-preview-artifact="${escapeHtml(item.uid)}">查看交付包</button></span></div>`;
			}
			sourceSummaryRow(item) {
				return `<div class="dsh-sdd-row"><span></span><span><strong>${escapeHtml(item.externalKey ?? item.uid)} · ${escapeHtml(item.title)}</strong><span class="dsh-sdd-muted">${escapeHtml({
					requirement: "需求",
					defect: "缺陷",
					issue: "问题"
				}[item.kind] ?? item.kind)} · ${escapeHtml(item.provider === "command" ? "业务适配器" : item.provider)} · 固定来源快照</span></span><span><span class="dsh-sdd-badge">外部内容</span><button class="dsh-sdd-button" data-preview-source="${escapeHtml(item.uid)}">查看完整内容</button></span></div>`;
			}
			...
			developmentHtml(snapshot) {
				...
				const repositoryRows = workspace?.repositories.map((repo) => {
					...
					const testState = currentTests.length === 0 ? "当前代码尚无测试证据" : currentTests.some((test) => !test.passed) ? `当前测试失败 ${currentTests.filter((test) => !test.passed).length} 项` : currentTests.every((test) => test.skipped) ? `已人工跳过测试：${currentTests.at(-1)?.description ?? ""}` : `当前测试通过 ${currentTests.filter((test) => !test.skipped).length} 项${currentTests.some((test) => test.skipped) ? "（含跳过说明）" : ""}`;
			...
			bind() {
				...
				root.querySelector("[data-action=\"work-item\"]")?.addEventListener("change", (event) => {
					...
					this.state.targetArtifactUid = void 0;
					this.state.selected = this.state.snapshot !== void 0 && stageMenu(this.state.menu) ? this.defaultInputs(this.state.snapshot, this.state.menu) : /* @__PURE__ */ new Set();
					this.render();
				});
				...
				root.querySelector("[data-action=\"import-standalone-defect\"]")?.addEventListener("click", () => {
					this.importSource("defect");
				});
				root.querySelector("[data-action=\"import-attached-defect\"]")?.addEventListener("click", () => {
					this.importSource("defect", this.state.workItemUid);
				});
				root.querySelector("[data-action=\"conversation\"]")?.addEventListener("click", () => {
					this.startConversation();
				});
				root.querySelector("[data-action=\"open-settings\"]")?.addEventListener("click", () => this.open("settings"));
				root.querySelector("[data-action=\"close-delivery\"]")?.addEventListener("click", () => {
					this.closeDelivery();
				});
				root.querySelectorAll("[data-product-file]").forEach((button) => button.addEventListener("click", () => {
					this.showProductFile(button.dataset.productFile, button.dataset.productTitle ?? "产品资料");
				}));
				root.querySelector("[data-action=\"apply-dashboard-filter\"]")?.addEventListener("click", () => {
					this.state.dashboardQuery = root.querySelector("[data-dashboard-query]")?.value ?? "";
					this.state.dashboardKind = root.querySelector("[data-dashboard-kind]")?.value ?? "all";
					this.state.dashboardStatus = root.querySelector("[data-dashboard-status]")?.value ?? "all";
					this.render();
				});
				root.querySelector("[data-dashboard-query]")?.addEventListener("keydown", (event) => {
					if (event.key === "Enter") root.querySelector("[data-action=\"apply-dashboard-filter\"]")?.click();
				});
				root.querySelector("[data-action=\"select-inputs\"]")?.addEventListener("click", () => {
					this.chooseInputs();
				});
				...
				root.querySelector("[data-action=\"configure-project-git\"]")?.addEventListener("click", () => {
					this.configureProjectGit();
				});
				root.querySelector("[data-action=\"project-git-fetch\"]")?.addEventListener("click", () => {
					this.mutate({
						kind: "project-git-fetch",
						workspaceId: this.state.workspaceId
					});
				});
				root.querySelector("[data-action=\"project-git-sync\"]")?.addEventListener("click", () => {
					this.syncProjectGit();
				});
				root.querySelector("[data-action=\"project-git-commit\"]")?.addEventListener("click", () => {
					this.commitProjectGit();
				});
				root.querySelector("[data-action=\"project-git-push\"]")?.addEventListener("click", () => {
					this.pushProjectGit();
				});
				root.querySelectorAll("[data-resolve-key-conflict]").forEach((button) => button.addEventListener("click", () => {
					this.resolveKeyConflict(button.dataset.resolveKeyConflict);
				}));
				...
				root.querySelector("[data-action=\"skip-stage\"]")?.addEventListener("click", () => {
					this.setStageApplicability("not-applicable");
				});
				root.querySelector("[data-action=\"restore-stage\"]")?.addEventListener("click", () => {
					this.setStageApplicability("applicable");
				});
				root.querySelector("[data-action=\"install-openspec\"]")?.addEventListener("click", () => {
					this.installOpenSpec();
				});
				root.querySelector("[data-action=\"enable-openspec\"]")?.addEventListener("click", () => {
					this.enableOpenSpec();
				});
				root.querySelector("[data-action=\"initialize-managed-openspec\"]")?.addEventListener("click", () => {
					this.initializeManagedOpenSpec();
				});
				root.querySelector("[data-action=\"create-managed-openspec-change\"]")?.addEventListener("click", () => {
					this.createManagedOpenSpecChange();
				});
				root.querySelector("[data-action=\"fork-managed-openspec-schema\"]")?.addEventListener("click", () => {
					this.forkManagedOpenSpecSchema();
				});
				root.querySelector("[data-action=\"validate-managed-openspec\"]")?.addEventListener("click", () => {
					if (this.state.workItemUid) this.mutate({
						kind: "openspec-validate",
						workspaceId: this.state.workspaceId,
						workItemUid: this.state.workItemUid
					});
				});
				root.querySelector("[data-action=\"open-managed-openspec\"]")?.addEventListener("click", () => {
					this.openManagedOpenSpec();
				});
				root.querySelectorAll("[data-openspec-edit]").forEach((button) => button.addEventListener("click", () => {
					this.editOpenSpecFile(button.dataset.openspecEdit);
				}));
				root.querySelector("[data-action=\"initialize-openspec\"]")?.addEventListener("click", () => {
					this.initializeOpenSpec();
				});
				root.querySelector("[data-action=\"view-openspec-templates\"]")?.addEventListener("click", () => {
					this.viewOpenSpecTemplates();
				});
				root.querySelector("[data-action=\"open-openspec-schema\"]")?.addEventListener("click", () => {
					this.openOpenSpecSchema();
				});
				root.querySelector("[data-action=\"fork-openspec-schema\"]")?.addEventListener("click", () => {
					this.forkOpenSpecSchema();
				});
				root.querySelector("[data-action=\"create-openspec-change\"]")?.addEventListener("click", () => {
					this.createOpenSpecChange();
				});
				root.querySelector("[data-action=\"disable-openspec\"]")?.addEventListener("click", () => {
					this.disableOpenSpec();
				});
				...
				root.querySelectorAll("[data-preview-source]").forEach((button) => button.addEventListener("click", () => {
					this.showSource(button.dataset.previewSource);
				}));
				...
				root.querySelectorAll("[data-dev-skip-test]").forEach((button) => button.addEventListener("click", () => {
					this.skipTest(button.dataset.devSkipTest);
				}));
				...
				root.querySelectorAll("[data-resolve-attached-removal]").forEach((button) => button.addEventListener("click", () => {
					this.resolveRemoval(button.dataset.resolveAttachedRemoval);
				}));
				root.querySelectorAll("[data-matrix-work-item]").forEach((button) => button.addEventListener("click", () => {
					...
					this.state.selected = artifact === void 0 && this.state.snapshot !== void 0 ? this.defaultInputs(this.state.snapshot, this.state.menu) : /* @__PURE__ */ new Set([...artifact?.basedOn.map((item) => item.uid) ?? [], ...artifact?.derivedFrom.map((item) => item.uid) ?? []]);
					this.syncMenus();
					this.render();
				}));
			}
			...
			async showProductFile(path, title) {
				try {
					const response = await call({
					...
					if (!response.ok) throw new Error(response.error);
					if (!("productFile" in response)) throw new Error("Host returned an unexpected product file");
					const backdrop = document.createElement("div");
					backdrop.className = "dsh-sdd-modal-backdrop";
					const html = purify.sanitize(k.parse(response.productFile.content));
					backdrop.innerHTML = `<section class="dsh-sdd-modal dsh-sdd-template-modal" role="dialog" aria-modal="true" aria-label="${escapeHtml(title)}"><header class="dsh-sdd-modal-header"><h2>${escapeHtml(title)}</h2><p class="dsh-sdd-muted">${escapeHtml(response.productFile.path)}</p></header><div class="dsh-sdd-modal-body"><article class="dsh-sdd-markdown">${html}</article></div><footer class="dsh-sdd-modal-footer"><button class="dsh-sdd-button primary" type="button" data-product-close>关闭</button></footer></section>`;
					this.container.appendChild(backdrop);
					const close = () => backdrop.remove();
					backdrop.querySelector("[data-product-close]").addEventListener("click", close);
					backdrop.addEventListener("click", (event) => {
						if (event.target === backdrop) close();
					});
				} catch (error) {
			...
			async closeDelivery() {
				const snapshot = this.state.snapshot;
				const workItem = snapshot?.workItems.find((item) => item.uid === this.state.workItemUid);
				if (snapshot === void 0 || workItem === void 0) return;
				const features = snapshot.productKnowledge?.features ?? [];
				const featureOptions = [{
					value: "__new__",
					label: "创建新的产品特性"
				}, ...features.filter((item) => item.status === "active").map((item) => ({
					value: item.uid,
					label: `${item.key} · ${item.name} · v${item.currentVersion}`
				}))];
				const values = await this.openForm({
					title: `交付收口 · ${workItem.key}`,
					description: "确认这次需求创建或改变的主要产品特性。系统会冻结全部历史成果、更新产品当前规格，并生成转测报告和邮件稿。",
					submitLabel: "确认归档并沉淀",
					fields: [
						{
							name: "feature",
						...
						},
						{
							...
							required: true,
							value: workItem.title,
							placeholder: "用面向产品的语言描述交付后的当前能力，不要只描述开发过程。"
						}
					]
				});
				if (values === void 0) return;
				const selected = String(values.feature);
				const existing = features.find((item) => item.uid === selected);
				const selectedChangeType = String(values.changeType);
				const changeType = existing === void 0 ? "created" : selectedChangeType === "deprecated" ? "deprecated" : "updated";
				await this.mutate({
					kind: "close-delivery",
					workspaceId: this.state.workspaceId,
					workItemUid: workItem.uid,
					...existing === void 0 ? {} : { featureUid: existing.uid },
					featureName: existing?.name ?? String(values.featureName),
					changeType,
			...
			showSource(sourceUid) {
				...
				const previewRecord = JSON.parse(raw);
				const backdrop = document.createElement("div");
				backdrop.className = "dsh-sdd-modal-backdrop";
				...
				this.container.appendChild(backdrop);
				const preview = backdrop.querySelector("[data-source-preview]");
				const render = (mode) => {
					preview.innerHTML = mode === "source" ? `<pre class="dsh-sdd-template-preview">${escapeHtml(raw)}</pre>` : "<div data-json-source></div>";
					if (mode === "preview") mountJsonPreview(preview.querySelector("[data-json-source]"), previewRecord);
					backdrop.querySelectorAll("[data-source-mode]").forEach((button) => button.classList.toggle("primary", button.dataset.sourceMode === mode));
				};
				render("preview");
				backdrop.querySelectorAll("[data-source-mode]").forEach((button) => button.addEventListener("click", () => render(button.dataset.sourceMode)));
				const close = () => backdrop.remove();
				backdrop.querySelector("[data-source-close]").addEventListener("click", close);
				backdrop.addEventListener("click", (event) => {
					if (event.target === backdrop) close();
				});
			}
			...
			async configureRepositoryScope() {
				...
				const values = await this.openForm({
					...
					submitLabel: "保存仓库范围",
					fields: repositories.map((repository) => ({
						name: `scope-${repository.id}`,
						label: `${repository.id} · ${repository.source} · 基线 ${repository.baseBranch}`,
						type: "checkbox",
						value: workItem.repositoryScope?.includes(repository.id) === true
					}))
				});
				if (values === void 0) return;
			...
			async inspectProjectRepository(source) {
				try {
					...
					const values = await this.openForm({
						title: "初始化空 Git 仓库",
						description: "仓库还没有任何提交。插件可以创建一个不包含现有文件的空初始提交，使基线分支和 Worktree 可用；未跟踪文件保持原样。如果仓库已有暂存文件，系统会拒绝自动提交。",
						submitLabel: "创建空初始提交",
						fields: [{
							name: "branch",
							label: "初始分支",
							...
							name: "confirmed",
							label: "我确认创建空初始提交（不会提交当前文件）",
							type: "checkbox",
							required: true
						}]
					});
					if (values?.confirmed !== true) return void 0;
			...
			async configureDevelopmentTargets() {
				...
				if (scope.length === 0) {
					this.state.error = "请先在本需求开发设置中勾选并保存代码仓库范围";
					return this.render();
				}
				const values = await this.openForm({
					title: `配置开发目标 · ${workItem.key}`,
					description: "选择本需求实际修改的仓库，并为每个选中的仓库填写具体、可交付的改动目标。",
					submitLabel: "保存开发设置",
					fields: [...scope.flatMap((repository) => [{
						...
						name: `detail-${repository.id}`,
						label: `${repository.id} 的具体开发目标`,
						type: "textarea",
						value: workItem.developmentTargetDetails?.[repository.id] ?? "",
						placeholder: "例如：修改订单退款接口、状态机和对应自动化测试。"
					}]), ...[]]
				});
				if (values === void 0) return;
				...
				if (developmentTargets.some((id) => developmentTargetDetails[id].trim() === "")) {
					this.state.error = "每个选中的代码仓库都必须填写具体开发目标";
					return this.render();
				}
				const openSpec = workItem.openSpec ?? { enabled: false };
				await this.mutate({
					kind: "update-work-item-settings",
					workspaceId: this.state.workspaceId,
					workItemUid: workItem.uid,
			...
			async setStageApplicability(status) {
				if (!stageMenu(this.state.menu) || this.state.workItemUid === void 0) return;
				...
				const values = await this.openForm({
					...
					fields: [{
						...
						label: "说明（可选）",
						type: "textarea",
						placeholder: "例如：纯后端缺陷，不涉及页面和交互变化。"
					}]
				});
				if (values === void 0) return;
				await this.mutate({
			...
			async enableOpenSpec() {
				const workItem = this.state.snapshot?.workItems.find((item) => item.uid === this.state.workItemUid);
				if (workItem === void 0) return;
				const values = await this.openForm({
					title: `启用 OpenSpec · ${workItem.key}`,
					description: "插件会先在 .sdd 中建立规划工作区，让五个阶段共同维护一个 Change；进入开发时自动迁移到隔离代码仓。现有业务适配器和来源协议不受影响。",
					submitLabel: "启用",
					fields: [{
						name: "schema",
						label: "工作流 Schema",
						...
						required: true,
						value: "spec-driven",
						help: "通常保持 spec-driven；初始化后可在插件内复制并定制。"
					}]
				});
				if (values === void 0) return;
				await this.mutate({
			...
			async initializeManagedOpenSpec() {
				const workItem = this.state.snapshot?.workItems.find((item) => item.uid === this.state.workItemUid);
				if (workItem === void 0) return;
				const values = await this.openForm({
					title: `初始化 OpenSpec 规划工作区 · ${workItem.key}`,
					description: "由插件执行官方初始化。规划阶段保存在可提交的 .sdd 目录中，不会修改业务代码仓；无需记忆命令。",
					submitLabel: "初始化",
					fields: [{
						name: "tools",
						label: "AI 工具集成",
						...
						options: [
							{
								value: "agents",
								label: "DSH / 共享 Agents（推荐）"
							},
							{
								value: "none",
								label: "仅创建 OpenSpec 结构"
							},
							{
								value: "all",
								label: "全部支持工具"
						...
						name: "confirmed",
						label: "我确认创建当前需求的 OpenSpec 规划文件",
						type: "checkbox",
						required: true
					}]
				});
			...
			async createManagedOpenSpecChange() {
				const snapshot = this.state.snapshot;
				const workItem = snapshot?.workItems.find((item) => item.uid === this.state.workItemUid);
				if (workItem === void 0) return;
				const schema = snapshot?.openSpecValidation[workItem.uid]?.schema ?? workItem.openSpec?.schema ?? "spec-driven";
				const suggested = workItem.key.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "") || `change-${workItem.uid.slice(0, 8)}`;
				const values = await this.openForm({
					title: `创建需求 Change · ${workItem.key}`,
					description: "创建后，各阶段 AI 会自动读取并同步相关 OpenSpec 产物。",
				...
				await this.mutate({
					...
					workspaceId: this.state.workspaceId,
					workItemUid: workItem.uid,
					changeId: String(values.changeId),
					schema: String(values.schema)
				});
			}
			async forkManagedOpenSpecSchema() {
				const workItem = this.state.snapshot?.workItems.find((item) => item.uid === this.state.workItemUid);
				if (workItem === void 0) return;
				const values = await this.openForm({
					title: "创建可编辑的项目 Schema",
					description: "复制当前 Schema 的完整定义和模板到当前 OpenSpec 工作区。之后可直接在插件中编辑并校验。",
					submitLabel: "复制并启用",
					fields: [{
						name: "schema",
						label: "新 Schema 名称",
						type: "text",
						required: true,
						value: `${workItem.key.toLowerCase().replace(/[^a-z0-9]+/g, "-")}-sdd`,
						help: "使用 kebab-case。"
					}]
				});
				if (values === void 0) return;
			...
			async editOpenSpecFile(path) {
				const workItemUid = this.state.workItemUid;
				if (workItemUid === void 0) return;
				this.updateBusy(true, `正在读取 OpenSpec 文件 ${path}…`);
				try {
					const response = await call({
					...
					if (!response.ok) throw new Error(response.error);
					if (!("openSpecFile" in response)) throw new Error("Host 未返回 OpenSpec 文件");
					const file = response.openSpecFile;
					this.updateBusy(false);
					const values = await this.openForm({
						title: `编辑 OpenSpec · ${file.path}`,
						description: "保存后建议立即执行“校验全部规格”。",
						submitLabel: "保存文件",
						fields: [{
							name: "content",
							label: file.path,
			...
			async viewOpenSpecTemplates() {
				if (this.state.targetArtifactUid === void 0) return;
				const workItem = this.state.snapshot?.workItems.find((item) => item.uid === this.state.workItemUid);
				const validation = workItem === void 0 ? void 0 : this.state.snapshot?.openSpecValidation[workItem.uid];
				if (workItem === void 0 || validation === void 0) return;
				const response = await call({
					kind: "development-inspect-openspec-templates",
					workspaceId: this.state.workspaceId,
					artifactUid: this.state.targetArtifactUid,
					schema: validation.schema ?? "spec-driven"
				});
				if (!response.ok) {
					this.state.error = response.error;
					return this.render();
				}
				if (!("openSpecTemplates" in response)) {
					this.state.error = "Host returned an unexpected OpenSpec template response";
					return this.render();
				}
			...
			async openOpenSpecSchema() {
				if (this.state.targetArtifactUid === void 0) return;
				const workItem = this.state.snapshot?.workItems.find((item) => item.uid === this.state.workItemUid);
				const schema = workItem === void 0 ? void 0 : this.state.snapshot?.openSpecValidation[workItem.uid]?.schema;
				if (schema === void 0 || schema === "spec-driven") return;
				const response = await call({
			...
			async createOpenSpecChange() {
				...
				const artifact = snapshot?.artifacts.find((item) => item.uid === this.state.targetArtifactUid);
				const workItem = snapshot?.workItems.find((item) => item.uid === artifact?.workItemUid);
				const validation = workItem === void 0 ? void 0 : snapshot?.openSpecValidation[workItem.uid];
				if (workItem === void 0 || validation === void 0) return;
				...
				const values = await this.openForm({
					...
					fields: [{
						...
						type: "text",
						required: true,
						value: workItem.openSpec?.changeId ?? defaultChangeId,
						placeholder: "例如：req-1024-refund"
					}, {
						name: "schema",
						label: "Schema",
						type: "select",
						required: true,
						value: workItem.openSpec?.schema ?? validation.schema ?? schemas[0],
				...
				return new Promise((resolve) => {
					...
					const fieldHtml = config.fields.map((field) => {
						...
						if (field.type === "checkbox") {
							...
							return `<div class="dsh-sdd-field"${show}>${field.detail === void 0 ? checkbox : `<div class="dsh-sdd-checkbox-detail">${checkbox}<button class="dsh-sdd-button" type="button" data-dialog-detail="${escapeHtml(field.name)}">查看详情</button></div>`}</div>`;
						}
					...
					config.fields.filter((field) => field.detail !== void 0).forEach((field) => backdrop.querySelector(`[data-dialog-detail="${field.name}"]`)?.addEventListener("click", () => field.detail()));
						...
						for (const field of config.fields) {
							if (field.type === "manual-items") {
								...
								if (items.some((item) => item.title === "")) {
									rowError(group, `每个${field.label.includes("缺陷") ? "缺陷" : "子需求"}都必须填写标题`);
									return;
								}
			...
			async configureProjectGit() {
				const collaboration = this.state.snapshot?.project?.collaboration ?? {
					remote: "origin",
					baseBranch: "main",
					syncStrategy: "ff-only",
					commitScope: "sdd"
				...
				const values = await this.openForm({
					...
					fields: [
						...
						},
						{
							...
							type: "text",
							required: true,
							value: collaboration.baseBranch,
							placeholder: "例如：main"
						},
						{
							name: "syncStrategy",
							...
							type: "select",
							required: true,
							value: collaboration.syncStrategy,
							options: [{
								value: "ff-only",
								label: "仅 Fast-forward（推荐）"
							}, {
								value: "manual",
								label: "手工同步"
							}]
						},
						{
							...
							type: "select",
							required: true,
							value: collaboration.commitScope,
							options: [{
								value: "sdd",
								label: "过程数据与产品资产（推荐）"
							}, {
				...
				await this.mutate({
					...
					baseBranch: String(values.baseBranch),
					syncStrategy: String(values.syncStrategy),
					commitScope: String(values.commitScope)
				});
			}
			async syncProjectGit() {
			...
			async commitProjectGit() {
				...
				const values = await this.openForm({
					...
					fields: [{
						...
						type: "textarea",
						required: true,
						placeholder: "例如：docs(sdd): 完成 PAY-381 系统设计"
					}]
				});
				if (values !== void 0) await this.mutate({
					kind: "project-git-commit",
					workspaceId: this.state.workspaceId,
					message: String(values.message)
				});
			}
			async pushProjectGit() {
				const git = this.state.snapshot?.projectRepository;
				if (await this.openForm({
					title: "Push SDD 项目分支",
			...
			async resolveKeyConflict(artifactUid) {
				const artifact = this.state.snapshot?.artifacts.find((item) => item.uid === artifactUid);
				if (artifact === void 0) return;
				if (await this.openForm({
			...
			async importSource(forcedKind, attachToWorkItemUid) {
				const snapshot = this.state.snapshot;
				...
				if (attachToWorkItemUid !== void 0 && parent === void 0) {
					this.state.error = "当前需求工作单元不存在，请刷新后重试";
					return this.render();
				}
				...
				const values = await this.openForm({
					title: parent === void 0 ? `获取并预览${defaultKind === "defect" ? "独立缺陷" : "外部业务内容"}` : `获取并预览本需求缺陷 · ${parent.key}`,
					description: parent === void 0 ? "插件先读取外部事实并展示完整内容，确认后才保存快照和创建独立工作单元。" : `确认导入后，缺陷会归入 ${parent.key} · ${parent.title}，不会创建另一套五阶段交付空间。`,
					submitLabel: "获取并预览",
					fields: [
						...forcedKind === void 0 ? [{
							name: "kind",
						...
						}] : [],
						{
							...
							options: providers.map((value) => ({
								value,
								label: value === "manual" ? "手工录入（无需适配器）" : value === "command" ? "业务适配器（command）" : `已安装适配器：${value}`
							})),
							help: "手工录入开箱即用；业务适配器可由插件源码统一提供，也可由当前项目自定义。两者都会生成相同的标准来源和工作单元。"
						},
						{
							name: "connector",
							label: "业务系统连接",
							...
							options: connectors.length === 0 ? [{
								...
								value: connector.id,
								label: `${connector.id}（${connector.scope === "plugin" ? "插件内置" : connector.overridden ? "项目覆盖" : "项目自定义"}）`
							})),
							help: "存在自定义连接时默认使用自定义适配器；插件 business/ 与项目 .sdd/business/ 使用相同格式，同名时采用项目配置。",
							showWhen: {
								field: "provider",
								value: "command"
							}
						...
						{
							name: "manualItems",
							label: defaultKind === "defect" ? "更多缺陷（可选）" : "子需求（可选）",
							type: "manual-items",
							help: defaultKind === "defect" ? "可以一次录入多条缺陷；每条分别填写编号、标题和不限行数的完整内容。" : "每个子需求分别填写编号、标题和不限行数的详细内容。留空时主需求本身形成一个工作单元。",
							showWhen: {
								field: "provider",
								value: "manual"
							}
				...
				if (provider === "command" && !connector) {
					this.state.error = "请先在插件 business/connectors/ 或项目 .sdd/business/connectors/ 配置业务系统连接";
					return this.render();
				}
				this.state.error = void 0;
				...
				try {
					...
					const response = await call({
						...
						...connector ? { connector } : {},
						...input === void 0 ? {} : { input },
						...attachToWorkItemUid === void 0 ? {} : { attachToWorkItemUid }
					});
					if (!response.ok) throw new Error(response.error);
					...
					const selected = await this.openForm({
						...
						fields: preview.items.map((item, index) => ({
							...
							detail: () => {
								this.showSourceImportDetail(preview.uid, item.identity);
							}
						}))
					});
					...
					shouldRender = true;
					if (preview.executionMode === "attached" && preview.parentWorkItemUid !== void 0) {
						this.state.workItemUid = preview.parentWorkItemUid;
						this.state.targetArtifactUid = void 0;
						this.state.selected = this.workItemSourceUids(applied.snapshot, preview.parentWorkItemUid, stageMenu(this.state.menu) ? this.state.menu : void 0);
					} else {
						...
						if (first !== void 0) {
							...
							this.state.selected = this.workItemSourceUids(applied.snapshot, first.uid);
							this.state.targetArtifactUid = void 0;
						}
					}
				} catch (error) {
					...
					else this.updateBusy(false);
				}
			}
			async showSourceImportDetail(previewUid, identity) {
				try {
					const response = await call({
					...
					const detail = response.sourceImportDetail;
					const backdrop = document.createElement("div");
					backdrop.className = "dsh-sdd-modal-backdrop";
					...
					this.container.appendChild(backdrop);
					const body = backdrop.querySelector("[data-import-detail-body]");
					...
					const sourceText = (value) => escapeHtml(JSON.stringify(value, null, 2) ?? String(value));
					const render = (mode) => {
						const columns = detail.previous === void 0 ? "1" : "2";
						if (mode === "source") body.innerHTML = `<div class="dsh-sdd-source-detail-grid" data-columns="${columns}"><section class="dsh-sdd-source-panel"><h3>本次获取的完整内容</h3><pre class="dsh-sdd-template-preview">${sourceText(detail.source)}</pre></section>${detail.previous === void 0 ? "" : `<section class="dsh-sdd-source-panel"><h3>当前已导入版本</h3><pre class="dsh-sdd-template-preview">${sourceText(detail.previous)}</pre></section>`}</div>${detail.root === void 0 ? "" : `<details class="dsh-sdd-source-panel" style="margin-top:12px"><summary>主事项背景 · JSON 源码</summary><pre class="dsh-sdd-template-preview">${sourceText(detail.root)}</pre></details>`}${relationSection}`;
						else {
							body.innerHTML = `<div class="dsh-sdd-source-detail-grid" data-columns="${columns}"><section class="dsh-sdd-source-panel"><h3>本次获取的完整内容</h3><div data-json-current></div></section>${detail.previous === void 0 ? "" : "<section class=\"dsh-sdd-source-panel\"><h3>当前已导入版本</h3><div data-json-previous></div></section>"}</div>${detail.root === void 0 ? "" : "<details class=\"dsh-sdd-source-panel\" data-json-root-details style=\"margin-top:12px\"><summary>主事项背景</summary><div data-json-root style=\"margin-top:10px\"></div></details>"}${relationSection}`;
							mountJsonPreview(body.querySelector("[data-json-current]"), detail.source);
							if (detail.previous !== void 0) mountJsonPreview(body.querySelector("[data-json-previous]"), detail.previous);
							if (detail.root !== void 0) {
								const rootDetails = body.querySelector("[data-json-root-details]");
								rootDetails.addEventListener("toggle", () => {
									const root = rootDetails.querySelector("[data-json-root]");
									if (rootDetails.open && root.dataset.jsonMounted !== "true") {
										root.dataset.jsonMounted = "true";
										mountJsonPreview(root, detail.root);
									}
								});
							}
						}
						backdrop.querySelectorAll("[data-import-detail-mode]").forEach((button) => button.classList.toggle("primary", button.dataset.importDetailMode === mode));
					};
					render("preview");
					backdrop.querySelectorAll("[data-import-detail-mode]").forEach((button) => button.addEventListener("click", () => render(button.dataset.importDetailMode)));
					const close = () => backdrop.remove();
					backdrop.querySelector("[data-source-detail-close]").addEventListener("click", close);
					backdrop.addEventListener("click", (event) => {
						if (event.target === backdrop) close();
					});
				} catch (error) {
			...
			async chooseInputs() {
				if (!stageMenu(this.state.menu) || this.state.targetArtifactUid !== void 0) return;
				const snapshot = this.state.snapshot;
				const workItem = snapshot?.workItems.find((item) => item.uid === this.state.workItemUid);
				if (snapshot === void 0) return;
				const sourceUids = workItem === void 0 ? /* @__PURE__ */ new Set() : this.workItemSourceUids(snapshot, workItem.uid, this.state.menu);
				const sources = (workItem === void 0 ? snapshot.sources : snapshot.sources.filter((item) => sourceUids.has(item.uid))).filter((item) => item.validationErrors.length === 0);
				const currentStageIndex = STAGES.findIndex((item) => item.id === this.state.menu);
				const artifacts = snapshot.artifacts.filter((item) => item.workItemUid === this.state.workItemUid && STAGES.findIndex((stage) => stage.id === item.stage) < currentStageIndex && item.status === "accepted").sort((left, right) => STAGES.findIndex((item) => item.id === left.stage) - STAGES.findIndex((item) => item.id === right.stage) || right.updatedAt.localeCompare(left.updatedAt));
				const values = await this.openForm({
					...
					fields: [...sources.map((source) => ({
						name: `input-${source.uid}`,
						label: `原始来源 · ${source.externalKey ?? source.uid} · ${source.title}`,
						type: "checkbox",
						value: this.state.selected.has(source.uid),
						help: `${source.kind} · ${source.provider}`
					})), ...artifacts.map((artifact) => ({
						name: `input-${artifact.uid}`,
						label: `${STAGES.find((item) => item.id === artifact.stage)?.label ?? artifact.stage} · ${artifact.key} v${artifact.version} · ${artifact.title}`,
						type: "checkbox",
						value: this.state.selected.has(artifact.uid),
						help: artifact.relativeDirectory
					}))]
				});
				if (values === void 0) return;
			...
			async startConversation() {
				if (!stageMenu(this.state.menu) || this.state.targetArtifactUid === void 0) {
			...
			async createDevelopment() {
				...
				if (repositories.length === 0) {
					this.state.error = existing.size > 0 ? "已添加全部目标代码仓库" : "请先在本需求开发设置中选择代码仓库并填写具体开发目标";
					return this.render();
				}
				const values = await this.openForm({
					title: "添加代码仓库",
			...
			async initializeOpenSpec() {
				if (this.state.targetArtifactUid === void 0) {
					this.state.error = "请先选择一个开发测试交付件";
					return this.render();
				}
				const snapshot = this.state.snapshot;
				const artifact = snapshot?.artifacts.find((item) => item.uid === this.state.targetArtifactUid);
				const workItem = snapshot?.workItems.find((item) => item.uid === artifact?.workItemUid);
				if (artifact === void 0 || workItem?.openSpec?.enabled !== true) return;
				const values = await this.openForm({
					...
					fields: [{
						...
						options: [
							{
								value: "agents",
								label: "DSH / 共享 .agents skills（推荐）"
							},
							{
								value: "agents,claude",
								label: "DSH + Claude Code"
							},
							{
								value: "agents,cursor",
								label: "DSH + Cursor"
							...
							{
								value: "all",
								label: "全部 OpenSpec 支持工具"
							},
							{
								value: "none",
								label: "只创建 OpenSpec 目录"
							}
						],
						help: "DSH 读取 .agents/skills。Slash commands 只由 OpenSpec 为支持命令形式的工具生成。"
					}, {
						name: "confirmed",
						label: "我确认在当前隔离代码空间中运行 OpenSpec CLI",
						type: "checkbox",
						required: true
					}]
				});
			...
			async installOpenSpec() {
				...
				if ((await this.openForm({
					...
					fields: [{
						name: "confirmed",
						label: "我确认在当前电脑全局安装 OpenSpec CLI",
						type: "checkbox",
						required: true
					}]
				}))?.confirmed !== true) return;
			...
			async runTest(repositoryId) {
				if (!this.state.targetArtifactUid) return;
				const snapshot = this.state.snapshot;
				const repository = (snapshot?.developmentWorkspaces.find((item) => item.artifactUid === this.state.targetArtifactUid))?.repositories.find((item) => item.id === repositoryId);
				const run = snapshot?.runs.find((item) => item.artifactUid === this.state.targetArtifactUid && item.status !== "completed" && item.sessionId !== void 0);
				if (repository === void 0) return;
				if (run?.sessionId === void 0) {
					this.state.error = "请先开始开发阶段对话，再让 AI 执行测试验证";
					return this.render();
				}
				const binding = this.sessions.binding(run.sessionId);
				if (binding === void 0) {
					this.state.error = "绑定的开发会话当前不可用，请先恢复对话";
					return this.render();
				}
				...
				try {
					...
					if (!accepted.ok) {
						this.state.error = `${accepted.error.code}: ${accepted.error.message}`;
						return this.render();
					}
					this.sessions.open(run.sessionId);
					this.close();
			...
			async skipTest(repositoryId) {
				...
				const values = await this.openForm({
					...
					fields: [{
						...
						type: "textarea",
						required: true,
						placeholder: "说明为什么当前变更不运行测试，以及采用了什么替代验证。"
					}]
				});
				if (values === void 0) return;
				await this.mutate({
			...
			async commit(repositoryId) {
				...
				const values = await this.openForm({
					...
					description: "仅当当前代码状态存在有效的通过或人工跳过证据时才允许提交。插件会暂存隔离开发空间中的全部变更并创建本地 Git 提交；不会自动推送或合并。",
					submitLabel: "提交代码",
					fields: [{
						name: "message",
						label: "提交说明",
			...
			async resolveRemoval(workItemUid = this.state.workItemUid) {
				if (workItemUid === void 0) return;
				const workItem = this.state.snapshot?.workItems.find((item) => item.uid === workItemUid);
				const values = await this.openForm({
					title: `处理外部${workItem?.kind === "defect" ? "缺陷" : "需求"}移除`,
