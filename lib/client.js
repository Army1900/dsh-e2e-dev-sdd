...
`;
      ...
      <div class="dsh-sdd-grid" style="margin-top:14px"><section class="dsh-sdd-card"><h2>质量与追踪</h2><p>来源追踪覆盖率：<strong>${dashboard.traceability}%</strong></p>${dashboard.blockers.length === 0 ? "<div class=\"dsh-sdd-empty\">当前没有结构化阻塞项</div>" : `<ul class="dsh-sdd-checks">${dashboard.blockers.map((item) => `<li data-fail>${escapeHtml(item)}</li>`).join("")}</ul>`}</section><section class="dsh-sdd-card"><h2>最近活动</h2>${dashboard.recentEvents.length === 0 ? "<div class=\"dsh-sdd-empty\">暂无事件</div>" : `<div class="dsh-sdd-list">${dashboard.recentEvents.slice(0, 10).map((event) => `<div class="dsh-sdd-row"><span></span><span><strong>${escapeHtml(event.subject)}</strong><span class="dsh-sdd-muted">${escapeHtml(event.type)} · ${escapeHtml(event.time)}</span></span></div>`).join("")}</div>`}</section></div>`;
			...
			sourceRow(item) {
				...
				const detail = item.validationErrors.length > 0 ? item.validationErrors.join("; ") : `${item.provider} · ${item.kind} · ${item.relativePath}`;
			...
			outputRow(item, snapshot) {
				...
				return `<div class="dsh-sdd-row">${selectable ? `<input type="radio" name="sdd-target" data-target="${escapeHtml(item.uid)}" ${this.state.targetArtifactUid === item.uid ? "checked" : ""}>` : "<span></span>"}<span><strong>${escapeHtml(item.key)} · ${escapeHtml(item.title)}</strong><span class="dsh-sdd-muted">v${escapeHtml(item.version)} · ${escapeHtml(item.relativeDirectory)}/${escapeHtml(item.entry)}</span>${checks}<div class="dsh-sdd-actions"><button class="dsh-sdd-button" data-quality="${escapeHtml(item.uid)}">质量检查 ${report?.score ?? 0}%</button>${run?.sessionId ? `<button class="dsh-sdd-button" data-resume="${escapeHtml(run.uid)}">恢复对话</button><button class="dsh-sdd-button" data-sync="${escapeHtml(run.uid)}">同步结论</button>` : ""}${selectable ? `<button class="dsh-sdd-button" data-accept="${escapeHtml(item.uid)}">验收</button>` : ""}${run !== void 0 && item.status === "accepted" ? `<button class="dsh-sdd-button" data-complete="${escapeHtml(run.uid)}">完成阶段运行</button>` : ""}</div></span><span><span class="dsh-sdd-badge">${escapeHtml(item.status)}</span>${run ? `<span class="dsh-sdd-badge">${escapeHtml(run.status)}</span>` : ""}</span></div>`;
			}
			...
			bind() {
				...
				root.querySelector("[data-action=\"reinitialize\"]")?.addEventListener("click", () => {
					if (window.confirm("现有 project.yaml 将先备份，再生成默认配置。是否继续？")) this.mutate({
						kind: "reinitialize",
						workspaceId: this.state.workspaceId
					});
				});
			...
			async createDraft() {
				...
				const title = window.prompt("交付件标题")?.trim();
				if (!title) return;
				const key = window.prompt("项目编号（留空则自动生成）")?.trim() || void 0;
			...
			async importSource() {
				const providers = this.state.snapshot?.sourceProviders ?? [];
				if (providers.length === 0) {
					this.state.error = "当前没有可用的 Source Provider";
					return this.render();
				}
				const sourceKind = window.prompt("来源类型", this.state.menu === "development" ? "defect" : "requirement")?.trim();
				if (!sourceKind) return;
				const configured = this.state.snapshot?.project?.sources[sourceKind];
				const provider = window.prompt(`Source Provider（可用：${providers.join(", ")}）`, configured?.provider ?? providers[0])?.trim();
				if (!provider) return;
				if (!providers.includes(provider)) {
					this.state.error = `Source Provider 未注册：${provider}`;
					return this.render();
				}
				const connector = provider === "command" ? window.prompt("Connector ID（对应 .sdd/business/connectors/<id>.yaml）", configured?.connector)?.trim() : configured?.connector;
				if (provider === "command" && !connector) return;
				const key = window.prompt("外部需求或缺陷编号")?.trim();
				if (!key) return;
				await this.mutate({
					kind: "import-source",
					workspaceId: this.state.workspaceId,
			...
			async accept(artifactUid) {
				const report = this.state.snapshot?.quality[artifactUid];
				if (report === void 0) return;
				const checklist = Object.fromEntries(report.checks.filter((item) => item.code.startsWith("checklist:")).map((item) => [`item-${item.code.split(":")[1]}`, true]));
				if (!window.confirm("确认已逐项核对阶段验收清单，并接受该版本作为下游正式输入？")) return;
				await this.mutate({
					kind: "accept",
					workspaceId: this.state.workspaceId,
					artifactUid,
			...
			async createDevelopment() {
				...
				const repositories = this.state.snapshot?.project?.development.repositories ?? [];
				const repositoryId = window.prompt(`代码仓库 ID（可用：${repositories.map((item) => item.id).join(", ")}）`, repositories[0]?.id)?.trim();
			...
			async runTest(repositoryId) {
				...
				const testId = window.prompt(`测试命令（可用：${repository?.testCommands.map((item) => item.id).join(", ") ?? ""}）`, repository?.testCommands[0]?.id)?.trim();
				if (!testId) return;
				await this.mutate({
			...
			async commit(repositoryId) {
				if (!this.state.targetArtifactUid) return;
				const message = window.prompt("提交说明")?.trim();
				if (!message || !window.confirm(`将在隔离代码空间执行 git add -A 和 git commit：\n${message}\n继续吗？`)) return;
				await this.mutate({
