...
`;
		...
		function icon(index) {
			return `<svg viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true"><rect x="3" y="3" width="14" height="14" rx="3"/><path d="M6 10h8M10 6v8" opacity="${index === 0 ? ".2" : index === 5 ? "1" : ".45"}"/></svg>`;
		}
		var SddWorkbench = class {
			...
			mountMenus() {
				...
				MENUS.forEach((menu, index) => {
					...
					if (button === void 0) {
						...
						button.innerHTML = `${icon(index)}<span>${menu.label}</span>`;
			...
			async refresh() {
				...
				try {
					...
					if (this.state.workItemUid === void 0 || !response.snapshot.workItems.some((item) => item.uid === this.state.workItemUid)) this.state.workItemUid = response.snapshot.workItems.find((item) => item.status !== "completed")?.uid;
				} catch (error) {
      ...
      <section class="dsh-sdd-card"><h2>阶段进度</h2><div class="dsh-sdd-stage-grid">${dashboard.stages.map((item) => {
					return `<div class="dsh-sdd-stat"><strong>${STAGES.find((value) => value.id === item.stage).label}</strong><span class="dsh-sdd-badge">${item.status}</span><div class="dsh-sdd-progress"><span style="width:${item.completion}%"></span></div><span class="dsh-sdd-muted">${item.completion}% · draft ${item.drafts} · accepted ${item.accepted}</span></div>`;
				}).join("")}</div></section>
      <div class="dsh-sdd-grid" style="margin-top:14px"><section class="dsh-sdd-card"><h2>质量与追踪</h2><p>来源追踪覆盖率：<strong>${dashboard.traceability}%</strong></p>${dashboard.blockers.length === 0 ? "<div class=\"dsh-sdd-empty\">当前没有结构化阻塞项</div>" : `<ul class="dsh-sdd-checks">${dashboard.blockers.map((item) => `<li data-fail>${escapeHtml(item)}</li>`).join("")}</ul>`}</section><section class="dsh-sdd-card"><h2>最近活动</h2>${dashboard.recentEvents.length === 0 ? "<div class=\"dsh-sdd-empty\">暂无事件</div>" : `<div class="dsh-sdd-list">${dashboard.recentEvents.slice(0, 10).map((event) => `<div class="dsh-sdd-row"><span></span><span><strong>${escapeHtml(event.subject)}</strong><span class="dsh-sdd-muted">${escapeHtml(event.type)} · ${escapeHtml(event.time)}</span></span></div>`).join("")}</div>`}</section></div>${this.traceabilityHtml(snapshot)}`;
			...
			traceabilityHtml(snapshot) {
				...
				return `<section class="dsh-sdd-card" style="margin-top:14px"><h2>交付追踪矩阵 · ${escapeHtml(workItem.key)}</h2><p class="dsh-sdd-muted">外部需求、五阶段交付件及其固定上游版本。</p><div class="dsh-sdd-list">${rows}</div></section>`;
			}
			...
			stageSettingsHtml(snapshot, stage) {
				...
				const action = stage === "architecture" ? "<button class=\"dsh-sdd-button\" data-action=\"add-repository\">添加项目代码仓库</button><button class=\"dsh-sdd-button\" data-action=\"configure-scope\">确认仓库范围</button>" : stage === "specification" ? "<button class=\"dsh-sdd-button\" data-action=\"configure-targets\">确认开发目标与 OpenSpec</button>" : "";
			...
			outputRow(item, snapshot) {
				...
				return `<div class="dsh-sdd-row">${selectable ? `<input type="radio" name="sdd-target" data-target="${escapeHtml(item.uid)}" ${this.state.targetArtifactUid === item.uid ? "checked" : ""}>` : "<span></span>"}<span><strong>${escapeHtml(item.key)} · ${escapeHtml(item.title)}</strong><span class="dsh-sdd-muted">v${escapeHtml(item.version)} · ${escapeHtml(item.relativeDirectory)}/${escapeHtml(item.entry)} · 文件 ${item.files.length}</span>${checks}<div class="dsh-sdd-actions"><button class="dsh-sdd-button" data-preview-artifact="${escapeHtml(item.uid)}">查看交付包</button><button class="dsh-sdd-button" data-quality="${escapeHtml(item.uid)}">质量检查 ${report?.score ?? 0}%</button>${run?.sessionId ? `<button class="dsh-sdd-button" data-resume="${escapeHtml(run.uid)}">恢复对话</button><button class="dsh-sdd-button" data-sync="${escapeHtml(run.uid)}">同步结论</button>` : ""}${selectable ? `<button class="dsh-sdd-button" data-accept="${escapeHtml(item.uid)}">验收</button>` : ""}${item.status === "accepted" ? `<button class="dsh-sdd-button" data-revision="${escapeHtml(item.uid)}">创建新修订</button>` : ""}${run !== void 0 && item.status === "accepted" ? `<button class="dsh-sdd-button" data-complete="${escapeHtml(run.uid)}">完成阶段运行</button>` : ""}</div></span><span><span class="dsh-sdd-badge">${escapeHtml(item.status)}</span>${run ? `<span class="dsh-sdd-badge">${escapeHtml(run.status)}</span>` : ""}</span></div>`;
			}
			...
			bind() {
				...
				root.querySelectorAll("[data-revision]").forEach((button) => button.addEventListener("click", () => {
					this.mutate({
						...
						workspaceId: this.state.workspaceId,
						artifactUid: button.dataset.revision
					});
				}));
			...
			showTemplate() {
				...
				backdrop.innerHTML = `<section class="dsh-sdd-modal dsh-sdd-template-modal" role="dialog" aria-modal="true" aria-label="${escapeHtml(stage.label)}交付件模板"><header class="dsh-sdd-modal-header"><h2>${escapeHtml(stage.label)} · 交付件模板</h2><p class="dsh-sdd-muted">创建草稿和阶段 AI 输出都使用这一份模板。AI 必须保留全部二级章节并把确认结论写入对应位置。</p></header><div class="dsh-sdd-modal-body"><pre class="dsh-sdd-template-preview">${escapeHtml(artifactTemplate(stage.id))}</pre></div><footer class="dsh-sdd-modal-footer"><button class="dsh-sdd-button primary" type="button" data-template-close>关闭</button></footer></section>`;
			...
			async showArtifact(artifactUid) {
				...
				try {
					...
					const inventory = artifact.files.map((file) => `${file.path} · ${file.kind} · ${file.size} bytes`).join("\n");
					const backdrop = document.createElement("div");
					backdrop.className = "dsh-sdd-modal-backdrop";
					backdrop.innerHTML = `<section class="dsh-sdd-modal dsh-sdd-template-modal" role="dialog" aria-modal="true"><header class="dsh-sdd-modal-header"><h2>${escapeHtml(artifact.key)} · v${escapeHtml(artifact.version)}</h2><p class="dsh-sdd-muted">${escapeHtml(artifact.relativeDirectory)} · 整包哈希 ${escapeHtml(artifact.contentHash ?? "尚未冻结")}</p></header><div class="dsh-sdd-modal-body"><strong>交付包文件</strong><pre class="dsh-sdd-template-preview">${escapeHtml(inventory || "无文件")}</pre><strong>${escapeHtml(artifact.entry)}</strong><pre class="dsh-sdd-template-preview">${escapeHtml(response.artifactFile.content)}</pre></div><footer class="dsh-sdd-modal-footer"><button class="dsh-sdd-button primary" type="button" data-artifact-close>关闭</button></footer></section>`;
					this.container.appendChild(backdrop);
					const close = () => backdrop.remove();
			...
			async configureRepositoryScope() {
				...
				if (repositories.length === 0) {
					this.state.error = "请先在 .sdd/project.yaml 配置项目可用代码仓库";
					return this.render();
				}
				const values = await this.openForm({
					title: `确认系统仓库范围 · ${workItem.key}`,
					description: "选择本需求可能影响的所有代码仓库。规格设计只能从这个范围中选择具体开发目标。",
					submitLabel: "保存仓库范围",
					fields: repositories.map((repository) => ({
						name: `repo-${repository.id}`,
				...
				const repositoryScope = repositories.filter((repository) => values[`repo-${repository.id}`] === true).map((repository) => repository.id);
			...
			async addProjectRepository() {
				const values = await this.openForm({
					title: "添加项目代码仓库",
					description: "把代码仓库加入项目目录后，系统设计可以将它纳入具体需求的仓库范围。测试命令仍可在 project.yaml 中补充。",
					submitLabel: "添加仓库",
					fields: [
						{
							name: "id",
			...
			async createDraft() {
				...
				const stage = STAGES.find((item) => item.id === this.state.menu);
				const values = await this.openForm({
					title: `创建${stage.label}交付件`,
					description: "当前勾选的上游交付件和外部来源会固定写入本版本的追踪关系。",
					submitLabel: "创建草稿",
					fields: [{
						name: "title",
						label: "交付件标题",
						...
						label: "项目编号",
						type: "text",
						placeholder: "留空则按项目编号规则自动生成",
						help: "编号只用于展示和外部沟通，内部身份使用不可变 UUID。"
					}]
				});
				if (values === void 0) return;
			...
			async importSource(forcedKind) {
				...
				const defaultProvider = configured !== void 0 && providers.includes(configured.provider) ? configured.provider : providers[0];
					...
					fields: [
						...
						}] : [],
						{
							...
							options: providers.map((value) => ({
								value,
								label: value === "command" ? "项目业务适配器（command）" : `已安装适配器：${value}`
							})),
							help: "获取方式负责从企业系统读取原始内容，不直接生成正式交付件。"
						},
						{
							name: "connector",
							label: "业务系统连接",
			...
			async startConversation() {
				...
				try {
					...
					if (artifact) binding.session.rename(`[SDD] ${artifact.key} ${artifact.title}`);
