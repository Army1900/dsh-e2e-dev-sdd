...
var SddProjectService = class {
	...
	dashboard(artifacts, sources, workItems, quality, workspaces, recentEvents) {
		...
		const blockers = [...workItems.filter((item) => item.change !== void 0).map((item) => `${item.key}：${item.status === "removed-pending" ? "外部需求已移除，等待确认" : `需求已变更，需重审 ${item.change.reviewRequiredStages.map((stage) => stageDefinition(stage).label).join("、")}`}`), ...Object.values(quality).flatMap((report) => report.checks.filter((item) => item.status === "failed").map((item) => `${stageDefinition(report.stage).label}：${item.label}`))].slice(0, 12);
...
//#endregion
export { STAGES, SddIdentifierRegistry, SddSourceRegistry, apply, isStageId, name, parseAction, stageDefinition, validateSourceBundle, validateSourceEnvelope };
