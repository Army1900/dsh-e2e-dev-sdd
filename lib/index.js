...
var SddProjectService = class {
	...
	async readStagedImport(workspaceId, previewUid) {
	...
	dashboard(artifacts, sources, workItems, quality, workspaces, recentEvents) {
		...
		const stages = STAGES.map((definition) => {
			...
			const status = accepted > 0 ? "completed" : deliveryWorkItems.length > 0 && notApplicable === deliveryWorkItems.length ? "not-applicable" : items.length === 0 ? "not-started" : reports.some((report) => report.ready) ? "ready-for-review" : failedChecks > 0 ? "blocked" : "in-progress";
			return {
				stage: definition.id,
				status,
				completion,
