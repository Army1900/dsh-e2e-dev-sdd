...
import { access, appendFile, copyFile, cp, lstat, mkdir, readFile, readdir, readlink, realpath, rename, stat, unlink, writeFile } from "node:fs/promises";
	...
	async commit(projectPath, project, message) {
		...
		if (collaboration.commitScope === "workspace") await run([
			...
			const paths = (await Promise.all([
				...
				"deliveries",
				".gitignore"
			].map(async (path) => await exists$2(join(projectPath, path)) ? path : void 0))).filter((path) => path !== void 0);
			if (paths.length > 0) await run([
				"git",
				"add",
				"-A",
				"--",
...
你不是开放式陪聊助手，而是当前阶段的结构化引导者。首次读取材料后，在内部判断本需求在本阶段属于轻量、标准或复杂，不向用户展示技术评分：
- 轻量：目标单一、边界清楚、影响局部时，只保留必要的目标、关键规则、例外和验收确认，避免仪式化追问。
- 标准：存在多个场景、角色或上下游时，按目标→范围→主流程→边界异常→验收的顺序推进。
- 复杂：涉及多角色、多系统、高风险、迁移、合规或大量未知项时，增加备选方案、依赖、风险、分阶段决策和验证策略。
每轮只推进当前最重要的一组决策，优先提出 1 个、最多 3 个聚焦问题；能从材料确定的内容直接整理，不要求用户重复陈述。回答采用“当前理解—需要确认—确认后的下一步”的自然语言结构，但不要暴露底层规划引擎、Schema、Change、CLI 或文件同步术语。
完成回复前重新读取交付件，确保本轮结论已经落盘。若工具受策略限制，用业务语言说明当前缺少什么以及用户可以怎么继续。`;
...
function managedOpenSpecProject(workspacePath, workItemUid) {
	return join(workspacePath, ".sdd", "openspec", workItemUid);
}
async function openSpecFiles(openSpecRoot) {
...
var SddProjectService = class {
	...
	* once the configured isolated repository exists, that repository copy becomes authoritative. */
	async openSpecWorkspace(snapshot, workItem, requireInitialized = false) {
		...
		const repository = configured.repositoryId === void 0 ? void 0 : snapshot.developmentWorkspaces.filter((item) => developmentArtifactUids.has(item.artifactUid)).flatMap((item) => item.repositories).find((item) => item.id === configured.repositoryId);
		if (repository !== void 0) {
			const projectRoot = resolve(repository.path);
			const openSpecRoot = resolve(projectRoot, configured.path ?? "openspec");
			if (!contained$1(projectRoot, openSpecRoot)) throw new Error("OpenSpec 路径超出隔离代码仓库");
			if (!requireInitialized || await exists(join(openSpecRoot, "config.yaml")) || await exists(join(openSpecRoot, "config.yml"))) return {
				projectRoot,
				openSpecRoot,
		...
		if (requireInitialized && !await exists(join(openSpecRoot, "config.yaml")) && !await exists(join(openSpecRoot, "config.yml"))) throw new Error("请先在插件中初始化当前需求的 OpenSpec 工作区");
		return {
			projectRoot,
			openSpecRoot,
			workspace: "planning"
	...
	async syncManagedOpenSpecToDevelopment(snapshot, workItem) {
		...
		await cp(source, target.openSpecRoot, {
			recursive: true,
			errorOnExist: true
		});
		await appendEvent(snapshot.workspace.path, "openspec.promoted-to-development", workItem.key, "development", {
			repositoryId: workItem.openSpec?.repositoryId,
			path: workItem.openSpec?.path ?? "openspec"
		});
	}
	/** Best-effort internal planning preparation. Normal users never need to enable or operate OpenSpec. */
	async prepareInternalPlanning(workspaceId, snapshot, workItem) {
		if (workItem.openSpec?.enabled === false) return snapshot;
		if (!(await this.openSpecCli(snapshot.workspace.path)).installed) return snapshot;
		try {
			if (workItem.openSpec === void 0) {
			...
			if (validation?.canInitialize === true || validation === void 0) {
			...
			await appendEvent(snapshot.workspace.path, "planning.engine-fallback", workItem.key, void 0, { reason: error instanceof Error ? error.message : String(error) });
		}
		return snapshot;
	}
	...
	async execute(action) {
		...
		if (action.kind === "development-create") {
			...
			if (workItem?.openSpec?.enabled === true && workItem.openSpec.repositoryId === action.repositoryId) {
				...
				if (updatedWorkItem !== void 0) await this.syncManagedOpenSpecToDevelopment(updated, updatedWorkItem);
			}
	...
	async validateOpenSpecSettings(workspacePath, project, workItems, artifacts, developmentWorkspaces) {
		...
		for (const workItem of workItems) {
			...
			const repository = configured.repositoryId === void 0 ? void 0 : developmentWorkspaces.filter((item) => developmentArtifactUids.has(item.artifactUid)).flatMap((item) => item.repositories).find((item) => item.id === configured.repositoryId);
			...
			if (repository === void 0 && managedReady) {
				...
				let openSpecArtifacts = [];
				if (cli.installed && configured.changeId !== void 0 && await exists(join(managedRoot, "changes", configured.changeId))) {
					const status = await runNative([
						...
						"--json",
						"--no-color"
					], managedProjectRoot, 3e4);
					if (status.exitCode === 0) try {
						openSpecArtifacts = (JSON.parse(status.stdout).artifacts ?? []).filter((item) => typeof item.id === "string").map((item) => ({
							id: String(item.id),
				...
				result[workItem.uid] = {
					status: "valid",
					message: `规划工作区已就绪；Schema ${configuredSchema}${configured.changeId === void 0 ? "；下一步创建当前需求 Change" : ""}`,
					schema: configuredSchema,
					availableSchemas: [.../* @__PURE__ */ new Set([
						"spec-driven",
						configuredSchema,
						...projectSchemas
					])].sort(),
					...configured.changeId === void 0 ? {} : {
						changeId: configured.changeId,
						changeExists: await exists(join(managedRoot, "changes", configured.changeId))
					},
					workspace: "planning",
					relativeRoot: relative(workspacePath, managedRoot).replaceAll("\\", "/"),
					files,
			...
			if (repository === void 0 && (configured.repositoryId === void 0 || configured.path === void 0)) {
				result[workItem.uid] = {
					status: "pending",
					message: "已启用；下一步在插件中初始化规划工作区",
			...
			if (repository === void 0 && (!project.development.repositories.some((item) => item.id === configured.repositoryId) || !(workItem.developmentTargets ?? []).includes(configured.repositoryId))) {
				result[workItem.uid] = {
					status: "pending",
					message: "规划阶段可先初始化；进入开发前再确认目标代码仓库",
					canInitialize: cli.installed,
					workspace: "planning",
					...cliFields
				};
			...
			if (repository === void 0) {
				result[workItem.uid] = {
					status: "pending",
					message: `已配置，创建开发空间后迁移规划内容；${cli.installed ? `CLI ${cli.version ?? "已安装"}` : "CLI 未安装"}`,
					canInitialize: cli.installed,
					workspace: "planning",
					...cliFields
				};
			...
			try {
				...
				let openSpecArtifacts = [];
				if (cli.installed && changeId !== void 0 && await exists(join(target, "changes", changeId))) {
					const status = await runNative([
						nativeExecutable("openspec"),
						"status",
						"--change",
				...
				result[workItem.uid] = cli.installed ? {
					status: "valid",
					message: `开发工作区已就绪；Schema ${configuredSchema}${changeId === void 0 ? "；尚未创建当前需求 Change" : ""}`,
					schema: configuredSchema,
					availableSchemas,
					...changeId === void 0 ? {} : {
						changeId,
					...
					workspace: "development",
					relativeRoot: relative(workspacePath, target).replaceAll("\\", "/"),
					files,
					artifacts: openSpecArtifacts,
					...cliFields
				} : {
	...
	async updateOpenSpecSettings(workspaceId, workItemUid, enabled, schema) {
		const snapshot = await this.requireSnapshot(workspaceId);
		const workItem = snapshot.workItems.find((item) => item.uid === workItemUid);
		...
		const openSpec = enabled ? {
			enabled: true,
			...(workItem.openSpec?.repositoryId ?? workItem.developmentTargets?.[0]) === void 0 ? {} : { repositoryId: workItem.openSpec?.repositoryId ?? workItem.developmentTargets?.[0] },
			path: workItem.openSpec?.path ?? "openspec",
			schema: normalizedSchema,
		...
		await appendEvent(snapshot.workspace.path, enabled ? "openspec.enabled" : "openspec.disabled", workItem.key, void 0, { schema: normalizedSchema });
	}
	async initializeManagedOpenSpec(workspaceId, workItemUid, tools) {
		const snapshot = await this.requireSnapshot(workspaceId);
		const workItem = snapshot.workItems.find((item) => item.uid === workItemUid);
		if (workItem === void 0) throw new Error(`work item not found: ${workItemUid}`);
		const target = await this.openSpecWorkspace(snapshot, workItem, false);
		...
		const initialized = await runNative([
			nativeExecutable("openspec"),
			"init",
			"--tools",
			normalizedTools,
		...
		await appendEvent(snapshot.workspace.path, "openspec.initialized", workItem.key, void 0, {
			workspace: target.workspace,
			tools: normalizedTools,
			version: cli.version
		});
	}
	async forkManagedOpenSpecSchema(workspaceId, workItemUid, schema) {
		const snapshot = await this.requireSnapshot(workspaceId);
		const workItem = snapshot.workItems.find((item) => item.uid === workItemUid);
		if (workItem === void 0) throw new Error(`work item not found: ${workItemUid}`);
		const name = schema.trim();
		...
		const forked = await runNative([
			...
			"schema",
			"fork",
			workItem.openSpec?.schema ?? "spec-driven",
			name,
			"--no-color"
		], target.projectRoot, 6e4);
		...
		await appendEvent(snapshot.workspace.path, "openspec.schema-forked", workItem.key, void 0, {
			schema: name,
			workspace: target.workspace
		});
	}
	async createManagedOpenSpecChange(workspaceId, workItemUid, changeId, schema) {
		const snapshot = await this.requireSnapshot(workspaceId);
		const workItem = snapshot.workItems.find((item) => item.uid === workItemUid);
		if (workItem === void 0) throw new Error(`work item not found: ${workItemUid}`);
		const normalizedChangeId = changeId.trim();
		...
		await appendEvent(snapshot.workspace.path, "openspec.change-created", workItem.key, void 0, {
			changeId: normalizedChangeId,
			schema: normalizedSchema,
			workspace: target.workspace
		});
	}
	async openSpecFileTarget(workspaceId, workItemUid, requestedPath) {
		const snapshot = await this.requireSnapshot(workspaceId);
		const workItem = snapshot.workItems.find((item) => item.uid === workItemUid);
		if (workItem === void 0) throw new Error(`work item not found: ${workItemUid}`);
		...
		if (normalized === "" || !contained$1(workspace.openSpecRoot, target)) throw new Error("OpenSpec 文件路径无效");
		if (!/\.(?:md|ya?ml|json|txt)$/i.test(target)) throw new Error("插件只允许管理 OpenSpec 文本、Markdown、YAML 和 JSON 文件");
		return {
			snapshot,
			workItem,
			target,
			root: workspace.openSpecRoot
		};
	}
	async readOpenSpecFile(workspaceId, workItemUid, path) {
		...
		return {
			path: relative(resolved.root, resolved.target).replaceAll("\\", "/"),
			content: await readFile(resolved.target, "utf8"),
			editable: true
		};
	}
	async writeOpenSpecFile(workspaceId, workItemUid, path, content) {
		...
		if (isConfig) await this.updateOpenSpecSettings(workspaceId, workItemUid, true, String(parse(content).schema));
		await appendEvent(resolved.snapshot.workspace.path, "openspec.file-updated", resolved.workItem.key, void 0, { path: relative(resolved.root, resolved.target).replaceAll("\\", "/") });
	}
	async validateManagedOpenSpec(workspaceId, workItemUid) {
		const snapshot = await this.requireSnapshot(workspaceId);
		const workItem = snapshot.workItems.find((item) => item.uid === workItemUid);
		if (workItem === void 0) throw new Error(`work item not found: ${workItemUid}`);
		const target = await this.openSpecWorkspace(snapshot, workItem, true);
		...
		await appendEvent(snapshot.workspace.path, "openspec.validated", workItem.key, void 0, { workspace: target.workspace });
	}
	async openManagedOpenSpecPath(workspaceId, workItemUid, path) {
		const snapshot = await this.requireSnapshot(workspaceId);
		const workItem = snapshot.workItems.find((item) => item.uid === workItemUid);
		if (workItem === void 0) throw new Error(`work item not found: ${workItemUid}`);
		const workspace = await this.openSpecWorkspace(snapshot, workItem, true);
		const target = path.trim() === "" ? workspace.openSpecRoot : resolve(workspace.openSpecRoot, path.replaceAll("\\", "/").replace(/^\/+/, ""));
		if (!contained$1(workspace.openSpecRoot, target) || !await exists(target)) throw new Error("OpenSpec 路径不存在或超出工作区");
		const response = await this.api.host.openPath(request({ path: target }), AbortSignal.timeout(15e3));
		if (!response.result.ok) throw new Error(`${response.result.error.code}: ${response.result.error.message}`);
	}
	async updateWorkItemSettings(workspaceId, workItemUid, repositoryScope, developmentTargets, developmentTargetDetails, openSpec) {
		...
		if (openSpec?.enabled === true) {
			if (openSpec.repositoryId !== void 0 && !targets.includes(openSpec.repositoryId)) throw new Error("internal planning repository must be a confirmed development target");
		...
		const updated = {
			...
			openSpec: openSpec?.enabled === true ? {
				enabled: true,
				...(openSpec.repositoryId ?? targets[0]) === void 0 ? {} : { repositoryId: openSpec.repositoryId ?? targets[0] },
				path: (openSpec.path ?? "openspec").trim(),
	...
	async listProductKnowledge(workspacePath) {
		const featuresRoot = join(join(workspacePath, "product"), "features");
		...
		if (await exists(featuresRoot)) for (const directory of await readdir(featuresRoot, { withFileTypes: true })) {
			...
			if (!await exists(path)) continue;
			try {
				const feature = parse(await readFile(path, "utf8"));
				if (feature.schema === "dsh-sdd/feature@1" && typeof feature.uid === "string") features.push({
					...feature,
					history: feature.history ?? [],
					relativeDirectory: relative(workspacePath, dirname(path)).replaceAll("\\", "/")
				});
			} catch {}
		}
		if (await exists(deliveriesRoot)) for (const directory of await readdir(deliveriesRoot, { withFileTypes: true })) {
			...
			try {
				...
				if (delivery.schema === "dsh-sdd/delivery-archive@1" && typeof delivery.uid === "string") deliveries.push({
					...delivery,
					relativeDirectory: relative(workspacePath, dirname(path)).replaceAll("\\", "/")
				});
			} catch {}
	...
	async writeProductBaseline(workspacePath, features) {
	...
	async closeDelivery(workspaceId, workItemUid, featureUid, featureName, changeType, summary) {
		const snapshot = await this.requireSnapshot(workspaceId);
		const workItem = snapshot.workItems.find((item) => item.uid === workItemUid);
		if (workItem === void 0 || workItem.executionMode === "attached") throw new Error("delivery work item not found");
		...
		if (acceptedDevelopment === void 0) throw new Error("accept the development delivery before closing the requirement");
		...
		if (invalidFrozen !== void 0) throw new Error(`repair frozen deliverable ${invalidFrozen.key} before archiving: ${invalidFrozen.validationErrors.join("; ")}`);
		const product = snapshot.productKnowledge ?? await this.listProductKnowledge(snapshot.workspace.path);
		const existingFeature = featureUid === void 0 ? void 0 : product.features.find((item) => item.uid === featureUid);
		if (featureUid !== void 0 && existingFeature === void 0) throw new Error(`product feature not found: ${featureUid}`);
		if (existingFeature === void 0 && changeType !== "created") throw new Error("a new feature must use the created change type");
		if (existingFeature !== void 0 && changeType === "created") throw new Error("an existing feature must be updated or deprecated");
		const now = (/* @__PURE__ */ new Date()).toISOString();
		...
		const feature = existingFeature === void 0 ? {
			schema: "dsh-sdd/feature@1",
			uid: randomUUID(),
			key: this.nextProductKey(product.features, "FEAT"),
			...
			createdAt: now,
			updatedAt: now,
			createdByWorkItemUid: workItem.uid,
			history: [],
			relativeDirectory: ""
		} : {
			...existingFeature,
			name: featureName.trim(),
			status: changeType === "deprecated" ? "deprecated" : "active",
			currentVersion: nextVersion(existingFeature.currentVersion),
			summary: summary.trim(),
			updatedAt: now
		};
		...
		for (const artifact of frozenArtifacts.sort((left, right) => left.createdAt.localeCompare(right.createdAt))) {
			...
			await cp(join(snapshot.workspace.path, artifact.relativeDirectory), join(deliveryRoot, archivePath), {
				recursive: true,
				errorOnExist: true
			});
			artifactRefs.push({
				...
				...artifact.contentHash === void 0 ? {} : { contentHash: artifact.contentHash },
		...
		const sourceRoot = join(deliveryRoot, "sources");
		await mkdir(sourceRoot, { recursive: true });
		for (const sourceUid of this.currentSourceUids(snapshot, workItem)) {
			const source = snapshot.sources.find((item) => item.uid === sourceUid);
			if (source !== void 0) await cp(join(snapshot.workspace.path, source.relativePath), join(sourceRoot, basename(source.relativePath)));
		}
		...
		const stageRows = STAGES.map((stage) => workArtifacts.filter((item) => item.stage === stage.id && item.status === "accepted").sort((left, right) => right.updatedAt.localeCompare(left.updatedAt))[0]).filter((item) => item !== void 0).map((item) => `| ${stageDefinition(item.stage).label} | ${item.key} | v${item.version} | ${item.contentHash ?? "未记录"} |`).join("\n");
		...
		await writeFile(join(featureRoot, "feature.yaml"), stringify({
			...feature,
			relativeDirectory: void 0
		}), "utf8");
		...
		const archive = {
			...
			artifactRefs,
			repositoryRefs,
			reportPath: "transfer-test-report.md",
			emailPath: "transfer-test-email.md",
			relativeDirectory: relative(snapshot.workspace.path, deliveryRoot).replaceAll("\\", "/")
		};
		await writeFile(join(deliveryRoot, "manifest.yaml"), stringify({
			...archive,
			relativeDirectory: void 0
		}), "utf8");
		const completed = {
		...
		for (const attached of snapshot.workItems.filter((item) => item.executionMode === "attached" && item.parentWorkItemUid === workItem.uid && item.status !== "completed")) await writeFile(join(snapshot.workspace.path, ".sdd", "work-items", attached.uid, "work-item.yaml"), stringify({
			...attached,
			status: "completed",
			updatedAt: now
		}), "utf8");
		await appendEvent(snapshot.workspace.path, "delivery.archived", deliveryKey, "development", {
			workItemUid: workItem.uid,
			featureKey: feature.key,
			featureVersion: feature.currentVersion
		});
	}
	async readProductFile(workspaceId, path) {
		const snapshot = await this.requireSnapshot(workspaceId);
		const root = resolve(snapshot.workspace.path);
		const target = resolve(root, path);
		if (!contained$1(root, target) || !contained$1(resolve(root, "product"), target) && !contained$1(resolve(root, "deliveries"), target)) throw new Error("product file path escapes product knowledge and delivery archives");
		if (!await exists(target) || !(await stat(target)).isFile()) throw new Error(`product file not found: ${path}`);
		if ((await stat(target)).size > OPEN_SPEC_TEXT_LIMIT) throw new Error("product file exceeds 1 MiB");
		return {
			path: relative(snapshot.workspace.path, target).replaceAll("\\", "/"),
	...
	async context(workspaceId, stage, artifactUid, artifactUids, sourceUids, preparedCodeReferences) {
		...
		const openSpecValidation = workItem === void 0 ? void 0 : snapshot.openSpecValidation[workItem.uid];
		const openSpecContext = workItem?.openSpec?.enabled !== true || openSpecValidation?.status !== "valid" || openSpecValidation.relativeRoot === void 0 ? "" : `内部规划工作区（不得主动向普通用户暴露底层 OpenSpec、Schema、Change 或 CLI 术语）：${fileMention(openSpecValidation.relativeRoot)}\n当前规划单元：${openSpecValidation.changeExists === true ? openSpecValidation.changeId : "尚未建立"}\n可用文件：\n${(openSpecValidation.files ?? []).map((path) => `- ${fileMention(join(openSpecValidation.relativeRoot, path))}`).join("\n") || "- 尚无规划产物"}\n首次回答前读取配置和已有规划文件。形成确定结论后，同时更新阶段成果与相关内部规划文件，不让用户重复转述；对用户只描述业务决策、阶段进度和下一步。`;
		return [
	...
	bindRuntime(sessionId, stage, workspacePath, project, artifact, development, workItem, openSpecValidation, codeReferences = []) {
		...
		const openSpecTarget = workItem?.openSpec?.enabled === true && openSpecValidation?.relativeRoot !== void 0 ? resolve(workspacePath, openSpecValidation.relativeRoot) : void 0;
		const openSpecRuntime = openSpecTarget === void 0 || openSpecValidation?.status !== "valid" ? "" : `内部结构化规划引擎已就绪。内部规划目录：${openSpecTarget}。本阶段同步重点：${{
		...
		}[stage]}。每轮形成确定结论时，同时更新绑定的阶段成果和相关内部规划文件；先读取已有文件并保留其他阶段内容。不得主动向普通用户提及 OpenSpec、Schema、Change、CLI、目录迁移或底层文件同步；对用户只呈现当前阶段的问题、已确认决策、风险、进度和自然语言的下一步。`;
		...
		this.sessionController?.bind({
			...
			openSpecDirectories: openSpecTarget === void 0 ? [] : [openSpecTarget],
	...
	return {
		...
		handler: async (req, res) => {
			...
			try {
				...
				if ("openSpecFile" in result) return writeJson(res, 200, {
					ok: true,
					openSpecFile: result.openSpecFile
				});
				if ("productFile" in result) return writeJson(res, 200, {
					ok: true,
					productFile: result.productFile
				});
...
var StageSessionController = class {
	...
	attach(spec) {
		...
		const allowedWriteRoots = [
			resolve(spec.artifactDirectory),
			...spec.developmentDirectories.map((path) => resolve(path)),
			...(spec.openSpecDirectories ?? []).map((path) => resolve(path))
		];
