...
import { SddIdentifierRegistry, SddSourceRegistry, validateSourceBundle, validateSourceEnvelope } from "./extensions.js";
import { createHash, randomUUID } from "node:crypto";
import { access, appendFile, copyFile, mkdir, readFile, readdir, realpath, unlink, writeFile } from "node:fs/promises";
...
var SddProjectService = class {
	...
	async snapshot(workspaceId) {
		...
		for (const run of runs) {
			...
			if (artifact !== void 0) this.bindRuntime(run.sessionId, run.stage, workspace.path, project, artifact, developmentWorkspaces.find((item) => item.artifactUid === artifact.uid), workItems.find((item) => item.uid === artifact.workItemUid));
		}
		const quality = {};
		const partial = {
			artifacts,
	...
	async createDraft(workspaceId, stage, title, requestedKey, basedOn, sourceUids, workItemUid) {
	...
	async readArtifactFile(workspaceId, artifactUid, requestedPath) {
		...
		if (file === void 0) throw new Error(`artifact file not found: ${requestedPath}`);
	...
	async previewSourceImport(workspaceId, providerName, kind, key, connector) {
	...
	bindRuntime(sessionId, stage, workspacePath, project, artifact, development, workItem) {
...
//#endregion
export { STAGES, STAGE_ARTIFACT_TEMPLATES, SddIdentifierRegistry, SddSourceRegistry, apply, artifactTemplate, isStageId, name, parseAction, stageDefinition, validateSourceBundle, validateSourceEnvelope };
