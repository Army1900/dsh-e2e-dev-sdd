...
async function worktreeFingerprint(path) {
	const files = [...new Set((await run([
		"git",
		"ls-files",
		"-c",
		"-o",
		"--exclude-standard",
		"-z"
	], path)).stdout.split("\0").filter(Boolean))].sort();
	const hashes = [];
	for (const file of files) {
...
var SddProjectService = class {
		...
		for (const workItem of workItems) {
			...
			try {
				...
				result[workItem.uid] = cli.installed ? {
					status: "valid",
					message: `配置目录与 CLI ${cli.version ?? ""} 已验证`.trim(),
					...cliFields
				} : {
					status: "pending",
	...
	async revisionPlan(snapshot, previous) {
		...
		const dependencyStages = /* @__PURE__ */ new Set([...previousByStage.keys(), ...Object.entries(snapshot.project.dependencies[previous.stage] ?? {}).filter(([, mode]) => mode === "required").map(([stage]) => stage)]);
		const basedOn = [];
		...
		const currentSourceUids = workItem === void 0 ? previous.derivedFrom.map((item) => item.uid) : [...new Set([workItem.sourceUid, workItem.bundleSourceUid].filter((uid) => uid !== void 0))];
	...
	staleInputLabels(snapshot, artifact) {
		...
		if (artifact.stage === "requirements" && artifact.workItemUid !== void 0) {
			const workItem = snapshot.workItems.find((item) => item.uid === artifact.workItemUid);
			if (workItem?.sourceUid !== void 0 && !artifact.derivedFrom.some((reference) => reference.uid === workItem.sourceUid)) labels.push("原始需求来源已更新");
		}
		return labels;
	}
	...
	bindRuntime(sessionId, stage, workspacePath, project, artifact, development, workItem, openSpecValidation) {
		...
		const openSpecRuntime = openSpecTarget === void 0 || openSpecValidation?.status !== "valid" ? "" : `OpenSpec 已启用并通过检查。OpenSpec 项目根目录：${dirname(openSpecTarget)}。官方生成的共享 skills 位于 ${join(dirname(openSpecTarget), ".agents", "skills")}；由于代码仓是当前 SDD 工作空间内的隔离 Worktree，执行 OpenSpec 工作流前必须先读取匹配的 openspec-*/SKILL.md 并遵循，在 OpenSpec 项目根目录运行 openspec 命令。`;
	...
	dashboard(artifacts, sources, workItems, quality, workspaces, recentEvents) {
		...
		const blockers = [
			...
			...artifacts.filter((item) => item.status === "accepted").flatMap((item) => this.staleInputLabels({
				artifacts,
				workItems
			}, item).map((label) => `${item.key} v${item.version}：${label}，需要创建变更修订`)),
		...
		const deliveryMatrix = workItems.map((workItem) => {
			const cells = STAGES.map((stage) => {
				...
				if (workItem.change?.reviewRequiredStages.includes(stage.id) === true || artifact !== void 0 && this.staleInputLabels({
					artifacts,
					workItems
				}, artifact).length > 0) status = "blocked";
