...
var SddProjectService = class {
	...
	projectGit;
	openSpecCliCache;
	constructor(api, sourceRegistry, sessionController, git = new GitDevelopmentService(), projectGit = new ProjectGitService()) {
		this.api = api;
	...
	dashboard(artifacts, sources, workItems, quality, workspaces, recentEvents) {
		const stages = STAGES.map((definition) => {
			...
			const status = accepted > 0 ? "completed" : workItems.length > 0 && notApplicable === workItems.length ? "not-applicable" : items.length === 0 ? "not-started" : reports.some((report) => report.ready) ? "ready-for-review" : failedChecks > 0 ? "blocked" : "in-progress";
			return {
				stage: definition.id,
				status,
				completion,
		...
		const deliveryMatrix = workItems.map((workItem) => {
			const cells = STAGES.map((stage) => {
				...
				else if (workItem.change?.reviewRequiredStages.includes(stage.id) === true || artifact !== void 0 && this.staleInputLabels({
					artifacts,
					workItems
		...
		return {
			overallCompletion: applicableCells.length === 0 ? 0 : Math.round(applicableCells.filter((cell) => cell.status === "completed").length / applicableCells.length * 100),
			stages,
			requirements: {
