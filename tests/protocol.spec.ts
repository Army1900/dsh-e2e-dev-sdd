...
describe('parseAction', () => {
  it('accepts typed stage actions', () => {
    ...
    expect(parseAction({ kind: 'update-project-collaboration', workspaceId: 'w1', remote: 'origin', baseBranch: 'main', syncStrategy: 'ff-only', commitScope: 'sdd' })).toMatchObject({ kind: 'update-project-collaboration' })
    expect(parseAction({ kind: 'project-git-fetch', workspaceId: 'w1' })).toMatchObject({ kind: 'project-git-fetch' })
    expect(parseAction({ kind: 'project-git-sync', workspaceId: 'w1' })).toMatchObject({ kind: 'project-git-sync' })
    expect(parseAction({ kind: 'project-git-commit', workspaceId: 'w1', message: 'docs: update SDD' })).toMatchObject({ kind: 'project-git-commit' })
    expect(parseAction({ kind: 'project-git-push', workspaceId: 'w1' })).toMatchObject({ kind: 'project-git-push' })
    expect(parseAction({ kind: 'resolve-artifact-key-conflict', workspaceId: 'w1', artifactUid: 'a1' })).toMatchObject({ kind: 'resolve-artifact-key-conflict' })
