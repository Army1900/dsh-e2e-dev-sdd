...
describe('GitDevelopmentService', () => {
  ...
  it('creates an isolated worktree, records current AI test evidence and commits changes', async () => {
    ...
    const project = { development: { workspaceRoot: '.sdd-workspaces', branchPattern: 'sdd/{artifactKey}/{repositoryId}', mergeStrategy: 'manual', repositories: [{ id: 'app', source, baseBranch: 'main' }] } } as unknown as ProjectConfig
    ...
    workspace = await service.recordAiTest(projectPath, artifact.uid, 'app', { command: 'test-command', description: 'Unit', exitCode: 0, output: 'passed', sessionId: 's1', passed: true })
    expect(workspace.repositories[0]!.tests?.[0]).toMatchObject({ source: 'ai-shell', passed: true, stale: false })
...
describe('ProjectGitService', () => {
  it('commits SDD files, pushes, fetches and fast-forwards remote collaboration changes', async () => {
    ...
    await mkdir(join(projectPath, 'product'), { recursive: true }); await writeFile(join(projectPath, 'product', 'product-specification.md'), '# Product\n')
    await mkdir(join(projectPath, 'deliveries'), { recursive: true }); await writeFile(join(projectPath, 'deliveries', 'DLV-0001.md'), '# Delivery\n')
    ...
    expect(git(projectPath, 'show', '--pretty=', '--name-only', 'HEAD')).toContain('product/product-specification.md')
    expect(git(projectPath, 'show', '--pretty=', '--name-only', 'HEAD')).toContain('deliveries/DLV-0001.md')
