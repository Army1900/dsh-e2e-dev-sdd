...
describe('StageSessionController', () => {
  it('installs a scoped stage prompt and enforces write and shell policy', async () => {
    ...
    controller.bind({ sessionId: 's1', stage: 'requirements', systemPrompt: 'BOUND', projectPath: '/project', artifactDirectory: '/project/.sdd/artifacts/requirements/a1', developmentDirectories: [] })
  ...
  it('preloads a persisted binding before a cold agent resumes', () => {
    ...
    const controller = new StageSessionController(ctx)
    controller.bind({ sessionId: 'cold', stage: 'architecture', systemPrompt: 'RESTORED', projectPath: '/project', artifactDirectory: '/project/.sdd/artifacts/architecture/a1', developmentDirectories: [] })
