...
export const STAGE_RUNTIMES: Readonly<Record<StageId, StageRuntimeDefinition>> = {
  requirements: {
    ...
    toolPolicy: { allowShell: false, writableArea: 'artifact-only', forbiddenTools: ['bash', 'terminal_open', 'terminal_send', 'terminal_signal'] },
  ...
  development: {
    ...
    systemPrompt: `${COMMON}\n\n当前阶段：开发测试。你的角色是开发工程师、测试工程师与 Reviewer。产品代码只能在绑定的 .sdd-workspaces 隔离目录中修改；所有 bash 调用必须显式把 workdir 设置为绑定的代码仓库目录。先读取规格，再实现、测试、审查差异，并把真实命令和结果同步到开发交付件。未经用户明确操作不得推送或合并。`,
  },
}
