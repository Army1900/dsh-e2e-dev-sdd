...
class SddWorkbench {
  ...
  private showSource(sourceUid: string): void {
    ...
    const record = typeof source.content === 'object' && source.content !== null && !Array.isArray(source.content) ? source.content as Record<string, unknown> : undefined
    const narrative = typeof source.content === 'string' ? source.content
      : ['description', 'body', 'detail', 'text'].map(key => record?.[key]).find((value): value is string => typeof value === 'string')
    ...
    const render = (mode: 'preview' | 'source') => {
      preview.innerHTML = mode === 'source' ? `<pre class="dsh-sdd-template-preview">${escapeHtml(raw)}</pre>`
        : `${narrative === undefined ? '' : `<article class="dsh-sdd-markdown">${markdownHtml(narrative)}</article>`}<h3>结构化内容</h3><pre class="dsh-sdd-template-preview">${escapeHtml(contentText)}</pre>`
  ...
  private async importSource(forcedKind?: string, attachToWorkItemUid?: string): Promise<void> {
    ...
    const values = await this.openForm({
      ...
      fields: [
        ...
        { name: 'connector', label: '业务系统连接', type: 'select', required: true, value: configured?.connector ?? connectors[0]?.id, options: connectors.length === 0 ? [{ value: '', label: '尚未配置业务连接' }] : connectors.map(connector => ({ value: connector.id, label: `${connector.id}（${connector.scope === 'plugin' ? '插件内置' : connector.overridden ? '项目覆盖' : '项目自定义'}）` })), help: '插件 business/ 与项目 .sdd/business/ 使用相同格式；同名时采用项目配置。', showWhen: { field: 'provider', value: 'command' } },
