...
- 系统设计在页面检查本地或远程代码仓库、从已有分支中选择基线，再勾选确认仓库范围；仓库可在尚未创建隔离开发空间时切换基线或移除。规格设计确认具体开发目标及可选 OpenSpec 仓库路径；未确认时阻止阶段对话和验收。
...
5. 系统设计阶段在页面添加项目代码仓库：填写本地路径或远程地址后，插件读取已有分支并以下拉框选择基线，均不会在此时复制或下载代码。仓库添加后立即展示，隔离开发空间创建前可以切换基线；在同一页面勾选并确认本需求可能涉及的范围。规格设计阶段再从该范围中确认实际开发目标，并可记录目标仓库中的 OpenSpec 相对路径。
