...
- 导入确认前可逐条查看完整正文、元数据、业务关系和当前已导入版本；存在自定义 Provider 或 Connector 时默认选择企业适配器，仍可切换到手工录入。预览和正文详情使用轻量项目上下文，不重复计算 Git、OpenSpec、质量与看板，前端也只更新局部忙碌状态。项目看板导入的缺陷是独立工作单元，从当前需求内导入的缺陷则保留自身编号和来源、但复用父需求的交付流程。
