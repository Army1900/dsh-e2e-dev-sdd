...
- 系统设计在页面检查本地或远程代码仓库、从已有分支中选择基线，再勾选确认仓库范围；本地空仓库可以在明确确认后自动创建不包含现有文件的空初始提交，已暂存文件不会被自动提交。仓库可在尚未创建隔离开发空间时切换基线或移除。规格设计确认具体开发目标及可选 OpenSpec 仓库路径；未确认时阻止阶段对话和验收。
