...
- 系统设计在页面检查本地或远程代码仓库、从已有分支中选择基线，再勾选确认仓库范围；本地空仓库可以在明确确认后自动创建不包含现有文件的空初始提交，已暂存文件不会被自动提交。仓库可在尚未创建隔离开发空间时切换基线或移除。规格设计确认具体开发目标及可选 OpenSpec；插件检查 CLI 与配置目录，允许用户安装、使用官方 CLI 初始化或不使用 OpenSpec继续开发。
...
- 开发测试阶段从选定基线创建 `sdd/...` 特性分支，支持 `.sdd-workspaces/` 隔离 Worktree/Clone、测试、Git 状态和本地提交。
...
5. 系统设计阶段在页面添加项目代码仓库：填写本地路径或远程地址后，插件读取已有分支并以下拉框选择基线。刚执行 `git init`、尚无提交的本地仓库会提示一键创建空初始提交；该提交不会包含未跟踪文件，存在暂存文件时必须由用户自行提交。空远程仓库需要先显式创建提交并 push。仓库添加后立即展示，隔离开发空间创建前可以切换基线；在同一页面勾选并确认本需求可能涉及的范围。
...
8. 返回阶段页面查看交付包和结构质量、确认阶段验收清单并接受版本。需要修改 accepted 版本时点击“检查变更 / 提出调整”：插件先展示来源、上游交付件和模板的哈希差异；存在差异时处理上游变更，没有差异时必须填写主动调整原因。新修订使用新的变更会话，名称包含变更类型与版本，旧会话和旧交付件保持只读。不再需要的草稿可移入 `.sdd/trash`。
9. 在开发测试阶段只为规格确认的目标仓库创建隔离代码空间；本地仓库创建 Worktree，远程仓库 Clone，并从所选基线提交创建独立的 SDD 特性分支。运行配置测试并提交到该特性分支后，由负责人显式推送、创建 PR/MR 并合入基线分支。
10. 在项目看板检查五阶段流转、需求燃起图、需求交付热力图、待处理变更、缺陷和测试状态；可点击热力图单元格进入具体需求的阶段工作台，然后提交 `.sdd/` 变更。
