...
4. 从页面顶部选择当前需求工作单元，勾选它的最新来源和已接受上游交付件，再创建本阶段草稿。
5. 选择草稿并开始阶段对话；插件在对应 Session 安装阶段 System Prompt 和工具 Guard。
6. 与 Agent 迭代，确定结论逐轮写入 `.sdd/work-items/<uid>/artifacts/<stage>/...`。
7. 返回阶段页面查看结构质量、确认阶段验收清单并接受版本。存在外部变更时，按页面列出的受影响阶段依次重新评审。
8. 在开发测试阶段创建隔离代码空间、运行配置测试并形成代码提交。
9. 在项目看板检查进度、待处理变更、追踪覆盖率、缺陷和测试状态，然后提交 `.sdd/` 变更。
