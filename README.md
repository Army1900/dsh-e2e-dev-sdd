...
- 系统设计在页面检查本地或远程代码仓库、从已有分支中选择基线，再勾选确认仓库范围；本地空仓库可以在明确确认后自动创建不包含现有文件的空初始提交，已暂存文件不会被自动提交。仓库可在尚未创建隔离开发空间时切换基线或移除。规格设计确认具体开发目标及可选 OpenSpec；插件检查 CLI 与配置目录，允许用户安装、使用官方 CLI 初始化或不使用 OpenSpec 继续开发。
...
9. 在开发测试阶段只为规格确认的目标仓库创建隔离代码空间；本地仓库创建 Worktree，远程仓库 Clone，并从所选基线提交创建独立的 SDD 特性分支。通过“让 AI 验证”由开发会话自行识别并运行相关测试；真实结果绑定当前代码状态，或由负责人填写原因明确跳过。证据有效后才能提交到特性分支，再由负责人显式推送、创建 PR/MR 并合入基线分支。
