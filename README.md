...
4. 从页面顶部选择当前需求工作单元，在任意阶段自由勾选它的最新来源和其他阶段已接受交付件；至少选择一项输入。无需某个阶段时直接标记“不适用”，以后仍可恢复。
5. 在任意阶段的“本需求开发设置”添加本地或远程代码仓库、选择已有基线、勾选当前需求涉及的范围，并为每个目标仓库填写具体改动目标。刚执行 `git init`、尚无提交的本地仓库可以在明确确认后创建空初始提交；空远程仓库仍需先显式创建提交并 push。
...
9. 在开发测试阶段只为需求开发设置中的目标仓库创建隔离代码空间；本地仓库创建 Worktree，远程仓库 Clone，并从所选基线提交创建独立的 SDD 特性分支。启用 OpenSpec 时先运行官方初始化；初始化只准备目录和 AI 工具集成，不会生成需求内容。随后选择官方或项目自定义 Schema，点击“创建当前需求 Change”，再让 AI 生成并维护其 proposal/specs/design/tasks。通过“让 AI 验证”运行相关测试；有效证据形成后才能提交。
