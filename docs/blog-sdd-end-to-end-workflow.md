...
“项目设置”同时维护外层 SDD 项目仓库协作，以及阶段可参考、开发阶段可修改的目标代码仓库与开发规则。外层仓库可以查看分支、upstream、ahead/behind 和文件状态，执行 Fetch、仅 Fast-forward 同步、按范围提交及用户确认后的 Push。项目关联后的代码仓库默认提供给前四阶段只读查看；远程仓库只 Clone 一份共享对象缓存，开发空间从该缓存创建特性分支 Worktree。

![项目设置：代码仓库、默认基线、开发空间与流程规则](assets/sdd-blog/11-project-settings.jpg)

登记仓库只保存来源和默认基线，不会立刻下载代码：

- 本地仓库：读取本地分支和 `origin` 跟踪分支；前四阶段只读参考，开发时创建 Git Worktree。
- 远程仓库：通过 `git ls-remote` 获取分支；首次阶段会话 Clone 一份 bare 共享缓存，非开发参考和开发 Worktree 共用对象库。
